...
Корневой компонент для подготовки к просмотру настраивается в `Program.Main` методе приложения с гибкостью для регистрации различных служб посредством внедрения зависимостей. Добавить службы в приложение [ Blazor WebAssembly ](https://docs.microsoft.com/aspnet/core/blazor/fundamentals/dependency-injection?view=aspnetcore-5.0#blazor-webassembly) можно на
