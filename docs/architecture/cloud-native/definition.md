...
Прерывать работу и текст десять из ваших коллег. Попросите их определить термин "облако машинного кода". Хороший шанс, вы получите десять различных ответов.
...
Облачные машинные технологии — это *скорость* и *гибкость*. Бизнес-системы развиваются от предоставления бизнес-возможностей оружие стратегического преобразования, которое ускоряет бизнес-скорость и рост. Крайне важно немедленно получить идеи на рынке.
...
| [Uber](https://eng.uber.com/micro-deploy/) | Содержит 1000 и более служб в рабочей среде. Развертывает несколько тысяч раз каждую неделю. |
| [WeChat](https://www.cs.columbia.edu/~ruigu/papers/socc18-final100.pdf) | Содержит 3000 и более служб в рабочей среде. Развертывает 1 000 раз в день. |
...
### <a name="the-twelve-factor-application"></a>Приложение Twelve-Factor
...
Во многих случаях, применимых к веб-приложениям, многие специалисты считают Twelve-Factor в качестве надежного фундамента для создания собственных облачных приложений. Системы, основанные на этих принципах, могут быстро развертывать и масштабировать и добавлять функции для быстрого реагирования на изменения рынка.

В следующей таблице показана методология Twelve-Factor.
...
| 8 | параллелизм | Службы масштабируются по большому количеству небольших идентичных процессов (копий) в отличие от масштабирования одного крупного экземпляра на наиболее мощном компьютере. |
...
В книге [за Пределами Twelve-Factorного приложения](https://content.pivotal.io/blog/beyond-the-twelve-factor-app)автор Кевин Хоффман сведения о каждом из первоначальных 12 факторов (написанных на 2011). Кроме того, он обсуждает три дополнительных фактора, отражающих сегодняшний современный дизайн облачных приложений.

|    |  Новый фактор | Описание  |
| :-------- | :-------- | :-------- |
| 13 | Решения с доступом через API | Сделайте все услуги. Предположим, ваш код будет использоваться интерфейсным клиентом, шлюзом или другой службой. |
...
| 15 | Проверка подлинности и авторизация  | Реализуйте удостоверение с самого начала. Рассмотрим функции [RBAC (Управление доступом на основе ролей)](/azure/role-based-access-control/overview) , доступные в общедоступных облаках.  |
...
Архитектура микрослужб перемещает систему из внутрипроцессного в необработанное сетевое взаимодействие. Что происходит, когда служба B не отвечает на сетевой вызов из службы A? Или что происходит, когда служба C становится временно недоступной и другие службы, вызывающие ее, блокируются?
...
> *Коэффициент \# 1 указывает "отдельную базу кода для каждой микрослужбы, которая хранится в собственном репозитории. Отслеживание версий позволяет выполнять развертывание в нескольких средах ".*
...
Платформа Microsoft .NET отлично подходит. Бесплатный и открытый исходный код содержит множество встроенных функций для упрощения разработки микрослужб. .NET является кросс-платформенным. Приложения можно создавать и запускать в Windows, macOS и большинстве разновидностей Linux.

.NET является высокопроизводительным и имеет более высокую оценку в сравнении с Node.js и другими конкурирующими платформами. Интересно, [течемповер](https://www.techempower.com/) проводил широкий набор [тестов производительности](https://www.techempower.com/benchmarks/#section=data-r17&hw=ph&test=plaintext) на многих платформах веб-приложений и платформах. .NET вычислено в 10 лучших Node.js и других конкурирующих платформах.
...
> *Коэффициент \# 2 указывает, что каждая микрослужба изолирует и упаковывает свои собственные зависимости, внося изменения, не влияя на всю систему.*
...
> *Фактор \# 9 указывает, что "экземпляры службы должны быть уничтожены, отпользующих быстрые запуски для повышения возможностей масштабируемости и корректного завершения работы, чтобы оставить систему в правильном состоянии. Контейнеры DOCKER вместе с Orchestrator по сути соответствуют этому требованию ".*

> *Коэффициент \# 8 указывает, что "службы масштабируются по большому количеству небольших идентичных процессов (копий) в отличие от масштабирования одного крупного экземпляра на наиболее мощном компьютере."*
...
Службы Azure Kubernetes Services подробно описаны в главе 2, *масштабировании Cloud-Native приложений*.
...
>*Фактор \# 6* указывает, что "каждая микрослужба должна выполняться в собственном процессе, изолированной от других работающих служб. Выносить требуется состояние для службы резервного копирования, такой как распределенный кэш или хранилище данных ".
...
Введите общепринятое практическое занятие [инфраструктуры как код](/azure/devops/learn/what-is-infrastructure-as-code)или IaC.
...
- Проверка изменений осуществляется с помощью проверок кода в запросах на вытягивание и политиках построения ветви.
