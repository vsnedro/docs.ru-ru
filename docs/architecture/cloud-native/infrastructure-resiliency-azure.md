...
[Избыточность](https://docs.microsoft.com/azure/architecture/guide/design-principles/redundancy) — один из способов обеспечения устойчивости приложений. Точный уровень избыточности зависит от бизнес-требований и влияет на стоимость и сложность системы. Например, развертывание в нескольких регионах является более затратным и сложным для управления, чем развертывание в одном регионе. Для управления отработкой отказа и восстановлением размещения потребуются рабочие процедуры. Дополнительные затраты и сложность могут быть выровнены для некоторых бизнес-сценариев, но не для других.
