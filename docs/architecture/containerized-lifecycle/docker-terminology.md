...
**Мультиархитектурный образ**: Для нескольких архитектур это функция, которая упрощает выбор соответствующего образа в соответствии с платформой, в которой работает Docker. Например, когда Dockerfile запрашивает базовый образ **FROM mcr.microsoft.com/dotnet/sdk:5.0** из реестра, фактически возвращается **5.0-nanoserver-20H2**, **5.0-nanoserver-2004** или **5.0-buster-slim** (в зависимости от ОС и версии среды, в которой работает Docker).
