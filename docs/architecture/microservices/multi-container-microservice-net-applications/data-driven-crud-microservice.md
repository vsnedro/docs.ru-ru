...
Чтобы реализовать простую микрослужбу CRUD с помощью Visual Studio и .NET, начните с создания простого проекта веб-API ASP.NET Core (работающего в .NET, чтобы его можно было запустить на узле Linux Docker), как показано на рисунке 6-6.
...
В упомянутом ранее классе `CatalogController` тип `CatalogContext` (который наследуется от типа `DbContext`) внедряется вместе с другими необходимыми объектами в конструкторе `CatalogController()`.

Важной конфигурацией для настройки в проекте веб-API является регистрация класса DbContext в контейнере IoC службы. Обычно это делается в классе `Startup` путем вызова метода `services.AddDbContext<CatalogContext>()` в методе `ConfigureServices()`, как показано в следующем **упрощенном** примере:
...
В настоящее время Swashbuckle состоит из пяти независимых внутренних пакетов NuGet в высокоуровневом метапакете [Swashbuckle.AspNetCore](https://www.nuget.org/packages/Swashbuckle.AspNetCore) для приложений ASP.NET Core.
