...
Внешние ресурсы, например базы данных SQL, время от времени могут быть недоступны. Если ресурс временно недоступен, приложение может использовать логику повторных попыток, чтобы избежать возникновения исключения. Такой подход часто называют _устойчивостью подключений_. Вы можете реализовать собственную логику [повторных попыток с экспоненциальной выдержкой](/azure/architecture/patterns/retry), при которой приложение пытается повторить операцию с экспоненциально растущим временем ожидания, пока не будет достигнуто максимальное число повторных попыток. В этом методе учитывается, что облачные ресурсы могут периодически становиться недоступными на несколько секунд по разным причинам, что может приводить к неудачному завершению некоторых запросов.
...
Чтобы настроить в приложении использование кэшированной версии службы и при этом разрешить службе получать экземпляр CatalogService, который требуется в ее конструкторе, необходимо добавить в ConfigureServices следующий код:
...
## <a name="getting-data-to-no-locblazor-no-locwebassembly-apps"></a>Получение данных для приложений Blazor WebAssembly

При создании приложений, использующих Blazor Server, можно использовать Entity Framework и другие технологии прямого доступа к данным, как описано выше в этой главе. Однако при создании приложений Blazor WebAssembly, таких как другие платформы SPA, потребуется другая стратегия доступа к данным. Как правило, эти приложения обращаются к данным и взаимодействуют с сервером через конечные точки веб-API.

Если данные или операции выполняются с учетом секрета, обязательно ознакомьтесь с разделом о безопасности в [предыдущей главе](develop-asp-net-core-mvc-apps.md) и защитите свои интерфейсы API от несанкционированного доступа.

Пример приложения Blazor WebAssembly можно найти в [эталонном приложении eShopOnWeb](https://github.com/dotnet-architecture/eShopOnWeb) в проекте BlazorAdmin. Этот проект размещается в веб-проекте eShopOnWeb и позволяет пользователям в группе "Администраторы" управлять элементами в хранилище. Снимок экрана приложения представлен на рисунке 8-3.
...
При выборке данных из веб-API в приложении Blazor WebAssembly вы просто используете экземпляр `HttpClient` как в любом приложении .NET. Основные этапы — создание запроса на отправку (если это необходимо, обычно для запросов POST или PUT), ожидание запроса, проверка кода состояния и десериализация ответа. Если вы собираетесь делать много запросов к определенному набору API, рекомендуется инкапсулировать их и настроить базовый адрес `HttpClient`. Таким образом, если необходимо изменить любой из этих параметров между средами, можно внести изменения только в одном месте. Необходимо добавить поддержку для этой службы в `Program.Main`:
...
Это можно сделать из любого компонента, в который внедрен `HttpClient`, при условии, что `HttpClient` не был добавлен в службы приложения с временем существования `Transient`. Каждая ссылка на `HttpClient` в приложении ссылается на один и тот же экземпляр, поэтому изменения в одном компоненте проходят через все приложение. Хорошим местом для выполнения этой проверки подлинности (а затем указания маркера) является общий компонент, такой как основная навигация по сайту. Дополнительные сведения об этом подходе см. в проекте `BlazorAdmin` [эталонного приложения ShopOnWeb](https://github.com/dotnet-architecture/eShopOnWeb).

Одним из преимуществ Blazor WebAssembly перед традиционными одностраничными приложениями JavaScript является то, что не нужно синхронизировать копии объектов передачи данных (DTO). Проект Blazor WebAssembly и проект веб-API могут совместно использовать одни и те же DTO в одном общем проекте. Это устраняет некоторые проблемы, связанные с разработкой одностраничных приложений.

Чтобы быстро получить данные из конечной точки API, можно использовать встроенный вспомогательный метод `GetFromJsonAsync`. Существуют аналогичные методы для POST, PUT и т. д. Ниже показано, как получить класс CatalogItem из конечной точки API с помощью настроенного `HttpClient` в приложении Blazor WebAssembly.
