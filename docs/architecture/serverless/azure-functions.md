...
Другим вариантом размещения для приложений-функций является план [Premium](https://docs.microsoft.com/azure/azure-functions/functions-premium-plan). Этот план предоставляет "постоянно работающий" экземпляр, чтобы избежать начальной загрузки, поддерживает дополнительные функции, такие как подключение к виртуальной сети, и работает на высококлассном оборудовании.
