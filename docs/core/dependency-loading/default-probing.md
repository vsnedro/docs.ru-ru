...
И, наконец, могут добавляться дополнительные зависимости из переменной среды `ADDITIONAL_DEPS`.  `dotnet.exe` также содержит необязательный параметр `--additional-deps` для задания этого значения при запуске приложения.

Свойства `APP_PATHS` и `APP_NI_PATHS` не заполняются по умолчанию и опускаются для большинства приложений.
