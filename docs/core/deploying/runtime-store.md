...
## <a name="aspnet-core-implicit-store-net-core-20-only"></a>Неявное хранилище ASP.NET Core (только для .NET Core 2.0)
...
> [!NOTE]
> В случае с приложениями с [автономным развертыванием](index.md#publish-self-contained) предполагается, что в целевой системе необязательно имеются требуемые пакеты манифеста. Таким образом, параметр **\<PublishWithAspNetCoreTargetManifest>** нельзя установить в значение `true` для автономного приложения.
