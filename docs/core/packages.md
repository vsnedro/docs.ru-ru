...
Добавив элемент `<NetStandardImplicitPackageVersion>` в файл проекта, который неявно определяет версию метапакета, можно указать версию платформы ниже версии метапакета. Элемент `<NetStandardImplicitPackageVersion>` применим только при нацеливании на .NET Core и .NET Standard. Например, приведенный ниже файл проекта является допустимым.
