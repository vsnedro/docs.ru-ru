...
Поведение наката по умолчанию определяется [правилами сопоставления](#matching-rules).
...
| `latestFeature` | Использует установленные набор возможностей и уровень обновления с самой высокой версией. Уровень обновления должен соответствовать запрошенному основному номеру версии и дополнительному номеру версии с набором возможностей и уровнем обновления, который соответствует указанному значению или превышает его. <br> Если не найдено, происходит сбой. |
| `latestMinor` | Использует установленные дополнительный номер версии, набор возможностей и уровень обновления с самой высокой версией. Уровень обновления должен соответствовать запрошенному основному номеру версии и дополнительному номеру версии с набором возможностей и уровнем обновления, который соответствует указанному значению или превышает его. <br> Если не найдено, происходит сбой. |
| `latestMajor` | Использует установленный пакет SDK для .NET Core с самой высокой версией, которая соответствует указанному значению или превышает его. <br> Если не найдено, происходит сбой. |
...
Позволяет управлять версией пакета SDK для проекта в одном месте, а не в отдельном проекте. Дополнительные сведения см. в статье [Как разрешаются ссылки на пакеты SDK проекта](/visualstudio/msbuild/how-to-use-project-sdk#how-project-sdks-are-resolved).
...
В следующем примере показано, как использовать наивысший установленный номер версии, который соответствует указанной версии или больше нее. Показанный JSON не позволяет использовать версию пакета SDK до 2.2.200 и разрешает 2.2.200 или любую более позднюю версию, включая 3.0.xxx и 3.1.xxx.
...
> [!NOTE]
> Правила сопоставления определяются точкой входа `dotnet.exe`, которая является общей для всех установленных сред выполнения .NET Core. Правила сопоставления для последней установленной версии среды выполнения .NET Core используются, если установлено сразу нескольких сред выполнения или если применятся файл *global.json*.
...
- Если файл *global.json* найден и в нем указана версия пакета SDK:
  ...
  - Если значение `rollForward` не задано, в качестве политики `rollForward` по умолчанию используется `latestPatch`. В противном случае проверьте каждое значение и их поведение в разделе [rollForward](#rollforward).
