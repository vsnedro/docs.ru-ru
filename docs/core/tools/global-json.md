...
С помощью файла *global.json* можно определить, какая версия пакета SDK для .NET используется при выполнении команд .NET CLI. Выбор пакета SDK для .NET не зависит от указания целевой среды выполнения проекта. Версия пакета SDK для .NET указывает, какие версии .NET CLI используются.
...
| `latestMajor` | Использует установленный пакет SDK для .NET с самой высокой версией, которая соответствует указанному значению или превышает его. <br> Если не найдено, происходит сбой. |
...
## <a name="globaljson-and-the-net-cli"></a>global.json и .NET CLI
