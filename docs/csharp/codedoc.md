...
Тег `<returns>` описывает возвращаемое значение объявленного метода.
...
- Как минимум, типы и их члены должны иметь тег `<summary>`, так как его содержимое требуется для IntelliSense.
