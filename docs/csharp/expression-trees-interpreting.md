...
Начнем с посещения каждого узла в простом дереве выражения.
...
> Здесь не используется `var` для объявления этого дерева выражения, так как это невозможно, поскольку правая часть назначения неявно типизирована.

`LambdaExpression` — это корневой узел. Для получения нужного кода в правой части оператора `=>` следует найти один из дочерних элементов `LambdaExpression`. Это необходимо сделать со всеми выражениями в этом разделе. Благодаря родительскому узлу можно найти возвращаемый тип `LambdaExpression`.
