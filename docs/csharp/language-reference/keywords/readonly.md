...
- В объявлении члена экземпляра в типе структуры `readonly` указывает на то, что член экземпляра не изменяет состояние структуры. Дополнительные сведения см. в разделе о [членах экземпляров `readonly`](../builtin-types/struct.md#readonly-instance-members) в статье [Типы структур](../builtin-types/struct.md).
