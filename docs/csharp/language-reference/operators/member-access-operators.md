...
> [!NOTE]
> В C# 8 оператор, [допускающий значение NULL](null-forgiving.md), завершает список предыдущих условных операций со значением NULL. Например, выражение `x?.y!.z` анализируется как `(x?.y)!.z`. Из-за этой интерпретации `z` вычисляется, даже если `x` — `null`, что может привести к <xref:System.NullReferenceException>.
...
Этот потокобезопасный способ гарантирует, что вызывается только `handler`, не имеющий значения NULL. Поскольку экземпляры делегата являются неизменяемыми, ни один из потоков не может изменить значение, на которое ссылается локальная переменная `handler`. В частности, если код, выполняемый другим потоком, отменяет подписку на событие `PropertyChanged` и событие `PropertyChanged` принимает значение `null` до вызова `handler`, значение, на которое ссылается `handler`, остается неизменным. Оператор `?.` вычисляет левый операнд не более одного раза, гарантируя, что его нельзя изменить на `null` после того, как он пройдет проверку как не имеющий значение NULL.
