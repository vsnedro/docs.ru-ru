...
> [!NOTE]
> В версиях C# ниже 8.0 локальные функции не могут содержать модификатор `static`. При использовании ключевого слова `static` возникает ошибка компилятора CS0106, "Модификатор "static" недопустим для этого элемента" или ошибка компилятора, указывающая, что следует использовать C# 8.0 или более позднюю версию.
