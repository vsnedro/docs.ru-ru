...
description: При передаче переменной типа значения в метод по значению в C# никакие изменения не влияют на исходные данные. Чтобы изменить значение, необходимо выполнить передачу по ссылке.
ms.date: 07/20/2015
helpviewer_keywords:
