...
description: Узнайте, как проецировать или заполнять граф объектов из XML в C#. В примере кода происходит заполнение графа объектов классами из примера файла XML.
ms.date: 07/20/2015
...
ms.locfileid: "87104617"
---
# <a name="how-to-project-an-object-graph-c"></a><span data-ttu-id="39112-104">Как проецировать граф объекта (C#)</span><span class="sxs-lookup"><span data-stu-id="39112-104">How to project an object graph (C#)</span></span>
<span data-ttu-id="39112-105">Данный раздел иллюстрирует способ проецирования, или наполнения, из XML графа объектов.</span><span class="sxs-lookup"><span data-stu-id="39112-105">This topic illustrates how to project, or populate, an object graph from XML.</span></span>  
  
## <a name="example"></a><span data-ttu-id="39112-106">Пример</span><span class="sxs-lookup"><span data-stu-id="39112-106">Example</span></span>  
 <span data-ttu-id="39112-107">В следующем коде происходит заполнение графа объектов классами `Address`, `PurchaseOrder` и `PurchaseOrderItem` из XML-документа [Пример XML-файла. Стандартный заказ на покупку (LINQ to XML)](./sample-xml-file-typical-purchase-order-linq-to-xml-1.md).</span><span class="sxs-lookup"><span data-stu-id="39112-107">The following code populates an object graph with the `Address`, `PurchaseOrder`, and `PurchaseOrderItem` classes from the [Sample XML File: Typical Purchase Order (LINQ to XML)](./sample-xml-file-typical-purchase-order-linq-to-xml-1.md) XML document.</span></span>  
...
```  
 ...
 <span data-ttu-id="39112-108">В данном примере результат запроса LINQ возвращается в виде <xref:System.Collections.Generic.IEnumerable%601> класса `PurchaseOrderItem`.</span><span class="sxs-lookup"><span data-stu-id="39112-108">In this example, the result of the LINQ query is returned as an <xref:System.Collections.Generic.IEnumerable%601> of `PurchaseOrderItem`.</span></span> <span data-ttu-id="39112-109">Элементы в классе `PurchaseOrder` относятся к типу <xref:System.Collections.Generic.IEnumerable%601> класса `PurchaseOrderItem`.</span><span class="sxs-lookup"><span data-stu-id="39112-109">The items in the `PurchaseOrder` class are of type <xref:System.Collections.Generic.IEnumerable%601> of `PurchaseOrderItem`.</span></span> <span data-ttu-id="39112-110">В коде используется метод расширения <xref:System.Linq.Enumerable.ToList%2A> для создания коллекции <xref:System.Collections.Generic.List%601> исходя из результатов запроса.</span><span class="sxs-lookup"><span data-stu-id="39112-110">The code uses the <xref:System.Linq.Enumerable.ToList%2A> extension method to create a <xref:System.Collections.Generic.List%601> collection from the results of the query.</span></span>  
  
 <span data-ttu-id="39112-111">Пример выводит следующие результаты:</span><span class="sxs-lookup"><span data-stu-id="39112-111">The example produces the following output:</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="39112-112">См. также раздел</span><span class="sxs-lookup"><span data-stu-id="39112-112">See also</span></span>
