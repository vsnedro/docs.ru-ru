...
> [!TIP]
> Рекомендуется проверять типы с помощью методов расширения <xref:Microsoft.CodeAnalysis.CSharpExtensions.IsKind%2A> (для C#) или <xref:Microsoft.CodeAnalysis.VisualBasicExtensions.IsKind%2A> (для VB).
