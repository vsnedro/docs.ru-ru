...
Автор типа может определить один или несколько методов `Deconstruct`, присваивающих значения любому количеству [переменных `out`](language-reference/keywords/out-parameter-modifier.md), которые представляют составляющие этот тип элементы данных. Например, следующий тип `Person` определяет метод `Deconstruct`, который деконструирует объект person в элементы, представляющие имя и фамилию:
...
*Сами по себе* кортежи можно использовать в качестве [параметров `out`](language-reference/keywords/out-parameter-modifier.md). Это не является неоднозначностью, как упоминается выше в разделе [Деконструирование](#deconstruction). В вызове метода необходимо описать только форму кортежа:
