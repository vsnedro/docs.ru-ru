...
- [Гкноаффинитизе](gcnoaffinitize-element.md), который управляет тем, привязаны ли потоки или КУЧИ сервера GC с ЦП. `enabled`Для использования параметра гчеапаффинитиземаск атрибут элемента [гкноаффинитизе](gcnoaffinitize-element.md) должен быть `false` (его значение по умолчанию **GCHeapAffinitizeMask** ).
