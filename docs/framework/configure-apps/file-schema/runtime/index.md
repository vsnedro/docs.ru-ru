...
## <a name="alphabetical-list-of-runtime-elements"></a>Алфавитный список \<runtime> элементов
...
|[\<GCHeapAffinitizeMask>](gcheapaffinitizemask-element.md)|Определяет сходство между кучами сборщика мусора и отдельными процессорами.|
|[\<GCHeapCount>](gcheapcount-element.md)|Указывает число куч/потоков, используемых для сборки мусора сервера.  |
|[\<GCLOHThreshold>](gclohthreshold-element.md)|Указывает пороговый размер, который приводит к тому, что объекты попадают в кучу больших объектов (LOH).|
|[\<GCNoAffinitize>](gcnoaffinitize-element.md)|Указывает, следует ли привязать потоки сервера GC с ЦП.|
