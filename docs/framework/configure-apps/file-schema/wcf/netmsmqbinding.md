...
|`QueueTransferProtocol`|Допустимое значение <xref:System.ServiceModel.QueueTransferProtocol>, задающее поставленный в очередь транспорт коммуникационного канала, используемый этой привязкой. MSMQ не поддерживает адресацию Active Directory при использовании протокола надежного обмена сообщениями SOAP. Поэтому не следует устанавливать этот атрибут в значение `Srmp` или, `Srmps` Если `useActiveDirectory` атрибуту присвоено значение `true` .|  
