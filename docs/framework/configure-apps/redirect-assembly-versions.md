...
Можно включить автоматическое перенаправление связывания, если ваше приложение нацелено на старые версии рамочного соглашения .NET. Это поведение по умолчанию можно переопределить, предоставив обязательную информацию о перенаправлении в файле app.config для любой сборки или выключив функцию перенаправления связывания. Для получения информации о том, как включить или выключить эту функцию, [см.](how-to-enable-and-disable-automatic-binding-redirection.md)

<a name="bypass_PP"></a>
### <a name="bypassing-publisher-policy"></a>Обход политики издателя
 При необходимости вы можете переопределить политику издателя в файле конфигурации приложения. Например, новые версии сборок, которые объявлены как поддерживающие обратную совместимость, могут все-таки нарушать работу приложения. Если вы хотите обойти политику издателя, добавьте элемент [ \<publisherPolicy>](./file-schema/runtime/publisherpolicy-element.md) [ \<в зависимый элементсборки>](./file-schema/runtime/dependentassembly-element.md) в файле конфигурации приложения, и установите атрибут **применения** **нет,** который переопределяет любые предыдущие настройки **да.**
...
## <a name="specifying-assembly-binding-in-configuration-files"></a>Указание привязки сборки в файлах конфигурации
 ...
 Чтобы связать сборку, необходимо указать строку "urn:schemas-microsoft-com:asm.v1" с атрибутом **xmlns** в [ \<теге сборкиBinding>.](./file-schema/runtime/assemblybinding-element-for-runtime.md)
...
### <a name="limiting-assembly--bindings-to-a-specific-version"></a>Ограничения привязок сборок определенной версией
 Можно использовать **атрибутна На** [ \<сборкеОбязательный элемент>](./file-schema/runtime/assemblybinding-element-for-runtime.md) элементв в файле конфигурации приложения, чтобы перенаправить обязательные ссылки на сборку в конкретную версию рамочного соглашения .NET. Этот необязательный атрибут содержит номер версии .NET Framework, к которой применяется перенаправление. Если атрибут **appliesTo** не указан, элемент [\<assemblyBinding>](./file-schema/runtime/assemblybinding-element-for-runtime.md) применяется ко всем версиям платформы .NET Framework.
...
- [\<связывающаярепрямая> элемент](./file-schema/runtime/bindingredirect-element.md)
