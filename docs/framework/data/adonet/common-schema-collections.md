...
|----------------|--------------|-----------------|  
|CollectionName|строка|Имя коллекции, которая передается в метод **GetSchema** для возвращения коллекции.|  
|нумберофрестриктионс|INT|Число ограничений, которые могут быть указаны для коллекции.|  
...
|QuotedIdentifierPattern|строка|Регулярное выражение, соответствующее идентификатору, заключенному в кавычки, и имеющее значение идентификатора без кавычек. Например, если источник данных использует двойные кавычки для идентификации заключенных в кавычки идентификаторов, это будет выглядеть следующим образом: "(([^ \\ "] &#124;\\ " \\ ") *) ".|  
...
|StringLiteralPattern|строка|Регулярное выражение, соответствующее строковому литералу, и имеющее одинаковое с ним значение. Например, если источник данных использует одинарные кавычки для задания строк, это будет так: "(' ([^ '] &#124; ' ') * ')" '|  
...
|ProviderDbType|INT|Значение типа, зависящее от поставщика, которое следует использовать при указании типа параметра. Например, SqlDbType.Money или OracleType.Blob.|  
...
|литералсуффикс|строка|К заданному литералу применяется суффикс.|  
...
|ColumnName|DataType|Описание|  
|----------------|--------------|-----------------|  
|ресерведворд|строка|Зарезервированное слово конкретного поставщика.|  
