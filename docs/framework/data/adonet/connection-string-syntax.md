...
> [!IMPORTANT]
> По умолчанию для `Persist Security Info` ключевого слова используется значение `false` . Значение `true` или `yes` позволяет получить из строки соединения конфиденциальные данные (в том числе идентификатор пользователя и пароль) после открытия соединения. `Persist Security Info`Установите значение, чтобы `false` убедиться, что ненадежный источник не имеет доступа к конфиденциальным данным строки подключения.  
...
При подключении к базе данных SQL Azure или к хранилищу данных SQL Azure и предоставлении имени входа в формате `user@servername` Убедитесь, что `servername` значение в имени входа соответствует значению, указанному для `Server=` .
...
### <a name="type-system-version-changes"></a>Изменения версий системы типов  
 `Type System Version`Ключевое слово в <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> определяет представление SQL Server типов на стороне клиента. Дополнительные сведения о ключевом слове <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> см. в разделе `Type System Version`.  
