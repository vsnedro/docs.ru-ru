...
## <a name="datetime"></a>Дата и время  
 ...
 Часть даты должна иметь следующий формат: `YYYY` - `MM` - `DD` , где `YYYY` — это четырехзначное значение года в диапазоне от 0001 до 9999, `MM` равно месяцу от 1 до 12 и `DD` является значением дня, допустимым для данного месяца `MM` .  
