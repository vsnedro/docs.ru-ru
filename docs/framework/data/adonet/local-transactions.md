...
## <a name="determining-the-transaction-type"></a>Определение типа транзакции  
 ...
 Каждый поставщик данных .NET Framework имеет собственный `Transaction` объект для выполнения локальных транзакций. Если требуется выполнить транзакцию в базе данных SQL Server, выбирается транзакция <xref:System.Data.SqlClient>. Для транзакции Oracle используйте поставщик <xref:System.Data.OracleClient>. Кроме того, существует <xref:System.Data.Common.DbTransaction> класс, доступный для написания независимого от поставщика кода, требующего транзакций.  
