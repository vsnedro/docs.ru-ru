...
description: Узнайте, как ADO.NET минимизирует затраты на открытие подключений с помощью SQL Server пула соединений, что сокращает затраты на новые подключения.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
> [!CAUTION]
> Настоятельно рекомендуется всегда закрывать соединение после его использования, чтобы оно вернулось в пул. Это можно сделать с помощью `Close` `Dispose` методов или объекта или `Connection` путем открытия всех соединений внутри `using` инструкции в C# или `Using` инструкции в Visual Basic. Соединения, которые явно не закрыты, нельзя добавить или вернуть в пул. Дополнительные сведения см. в разделе [Использование инструкции](../../../csharp/language-reference/keywords/using-statement.md) или [инструкции: Удаление системного ресурса](../../../visual-basic/programming-guide/language-features/control-flow/how-to-dispose-of-a-system-resource.md) для Visual Basic.  
