...
### <a name="connection-pooling"></a>Объединение подключений в пул  
 Как и любые другие подключения, подключения, для которых включен режим MARS, упорядочиваются в пулы. Если приложение открывает два подключения (одно с включенным режимом MARS и другое, для которого режим MARS отключен), эти два подключения помещаются в отдельные пулы. Дополнительные сведения см. в разделе [Пулы подключений SQL Server (ADO.NET)](../sql-server-connection-pooling.md).  
