...
В службах [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] объектная модель, выраженная в языке программирования разработчика, сопоставляется с моделью данных реляционной базы данных. После этого операции с данными выполняются в соответствии с объектной моделью.  
...
## <a name="linq-to-sql-associations-and-database-foreign-key-relationships"></a>Связи и отношения внешнего ключа баз данных LINQ to SQL  
 ...
 В [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] можно представить ассоциации базы данных (например, связи внешнего ключа с первичным ключом), применяя <xref:System.Data.Linq.Mapping.AssociationAttribute> атрибут. В следующем сегменте кода `Order` класс содержит `Customer` свойство с <xref:System.Data.Linq.Mapping.AssociationAttribute> атрибутом. Это свойство и его атрибут предоставляют класс `Order` с отношением для класса `Customer`.  
