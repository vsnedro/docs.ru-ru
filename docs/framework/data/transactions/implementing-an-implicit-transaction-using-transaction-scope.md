...
description: Реализуйте неявную транзакцию с помощью класса TransactionScope в .NET. Этот класс предоставляет способ пометки блока кода как участвующего в транзакции.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
## <a name="creating-a-transaction-scope"></a>Создание области транзакции  
 ...
 Область транзакции запускается после создания нового <xref:System.Transactions.TransactionScope> объекта.  Как показано в примере кода, рекомендуется создавать области с помощью `using` инструкции. `using`Инструкция доступна как в C#, так и в Visual Basic и работает подобно `try` блоку..., `finally` чтобы обеспечить правильную удаленность области.  
...
## <a name="completing-a-transaction-scope"></a>Завершение области транзакции  
 ...
 Если <xref:System.Transactions.TransactionScope> объект изначально создал транзакцию, фактическая работа по фиксации транзакции диспетчером транзакций происходит после последней строки кода в `using` блоке. Если транзакция создана не этим объектом, фиксация происходит при каждом вызове метода <xref:System.Transactions.CommittableTransaction.Commit%2A> владельцем объекта <xref:System.Transactions.CommittableTransaction>. На этом этапе диспетчер транзакций вызывает диспетчеры ресурсов и уведомляет их о фиксации или откате в зависимости от того, <xref:System.Transactions.TransactionScope.Complete%2A> был ли вызван метод для <xref:System.Transactions.TransactionScope> объекта.  
  
 `using`Оператор гарантирует, что <xref:System.Transactions.TransactionScope.Dispose%2A> метод <xref:System.Transactions.TransactionScope> объекта будет вызываться даже при возникновении исключения. Вызов метода <xref:System.Transactions.TransactionScope.Dispose%2A> отмечает конец области транзакции. Исключения, возникающие после вызова данного метода, могут не воздействовать на транзакцию. Данный метод также восстанавливает предыдущее состояние внешней транзакции.  
...
## <a name="managing-transaction-flow-using-transactionscopeoption"></a><a name="ManageTxFlow"></a>Управление потоком транзакций с помощью Трансактионскопеоптион  
...
- не принимать участие в транзакции, в результате внешняя транзакция отсутствует.  
 ...
 Если область создана со значением <xref:System.Transactions.TransactionScopeOption.Suppress>, она не принимает участие в транзакции независимо от существования внешней транзакции. Область, созданная с этим значением, всегда имеет `null` в качестве окружающей транзакции.  
...
```  
 ...
 В примере показан блок кода без внешней транзакции, создающий новую область (`scope1`) со значением <xref:System.Transactions.TransactionScopeOption.Required>. Область `scope1` является корневой, поскольку она создает новую транзакцию (транзакцию A) и делает ее внешней транзакцией. `Scope1`затем создает еще три объекта, каждый из которых имеет другое <xref:System.Transactions.TransactionScopeOption> значение. Например, объект `scope2` создается со значением <xref:System.Transactions.TransactionScopeOption.Required>; поскольку существует внешняя транзакция, этот объект присоединяется к первой транзакции, созданной объектом `scope1`. Обратите внимание, что `scope3` является корневой областью новой транзакции, а `scope4` не имеет внешней транзакции.  
