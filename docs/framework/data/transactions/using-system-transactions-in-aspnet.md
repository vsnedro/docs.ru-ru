...
## <a name="dynamic-compilation"></a>Динамическая компиляция
 Чтобы импортировать и использовать <xref:System.Transactions> в приложении ASP.NET, динамически компилируемом во время доступа, в файле конфигурации необходимо указать ссылку на сборку <xref:System.Transactions>. В частности, ссылка должна добавляться в `compilation/assemblies` раздел корневого **Web.config** по умолчанию файла конфигурации или конкретного файла конфигурации веб-приложения. Это продемонстрировано в следующем примере.
