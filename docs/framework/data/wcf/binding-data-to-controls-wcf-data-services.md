---
description: 'Дополнительные сведения: привязка данных к элементам управления (службы данных WCF)'
...
ms.locfileid: "99766482"
---
# <a name="binding-data-to-controls-wcf-data-services"></a>Привязка данных к элементам управления (службы данных WCF)

[!INCLUDE [wcf-deprecated](~/includes/wcf-deprecated.md)]

С помощью службы данных WCF можно привязать элементы управления, такие как `ComboBox` и, `ListView` к экземпляру <xref:System.Data.Services.Client.DataServiceCollection%601> класса. Эта коллекция, наследуемая от <xref:System.Collections.ObjectModel.ObservableCollection%601> класса, содержит данные из канала Open Data Protocol (OData). Этот класс представляет коллекцию динамических данных, обеспечивающих выдачу уведомлений при добавлении и удалении элементов. При использовании экземпляра <xref:System.Data.Services.Client.DataServiceCollection%601> для привязки данных службы данных WCF клиентские библиотеки обрабатывали эти события, чтобы убедиться, что объекты, которые отписываются по, <xref:System.Data.Services.Client.DataServiceContext> остаются синхронизированными с данными в связанном ЭЛЕМЕНТЕ пользовательского интерфейса.  
...
> [!NOTE]
> Службы данных WCF не выполняет проверку пользовательских поведений, реализованных в этих делегатах.  
