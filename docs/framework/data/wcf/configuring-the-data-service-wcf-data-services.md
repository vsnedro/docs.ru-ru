...
|<xref:System.Data.Services.DataServiceBehavior.MaxProtocolVersion%2A>|Определяет версию протокола Atom, используемую службой данных. Если значение параметра <xref:System.Data.Services.DataServiceBehavior.MaxProtocolVersion%2A> равно значению, меньшему максимальному значению <xref:System.Data.Services.Common.DataServiceProtocolVersion> , то последняя функциональность WCF Data Services недоступна для клиентов, обращающихся к службе данных. Дополнительные сведения см. в разделе [Управление версиями службы данных](data-service-versioning-wcf-data-services.md).|  
