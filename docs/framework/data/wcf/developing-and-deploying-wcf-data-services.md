...
Рассмотрим следующее при разработке службы данных:

- Если вы планируете проверить подлинность пользователей или ограничить доступ для конкретных пользователей, определите требования безопасности службы данных. Дополнительные сведения см. в разделе [Securing WCF Data Services](securing-wcf-data-services.md).

- Программа проверки HTTP может быть полезна при отладке службы данных, позволяя вам проверять содержимое сообщений запросов и ответов. Любой планировщик сетевых пакетов, способный отображать необработанные пакеты, можно использовать для проверки HTTP-запросов к службе данных и ответов от нее.
...
Рассмотрим следующее при развертывании службы данных:
