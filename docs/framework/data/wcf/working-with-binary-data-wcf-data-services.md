...
Клиентская библиотека службы данных WCF позволяет извлекать и обновлять двоичные данные из канала Open Data Protocol (OData) одним из следующих способов:
...
Службы данных WCF реализует потоковую передачу двоичных данных с помощью HTTP, как определено в OData. В этом подходе двоичные данные обрабатываются как медиаресурс, отделенный, но при этом связанный с сущностью, которая называется медиассылкой. Дополнительные сведения см. в разделе [Streaming Provider](streaming-provider-wcf-data-services.md).
