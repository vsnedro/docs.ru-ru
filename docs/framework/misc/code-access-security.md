...
> [!NOTE]
> Поскольку управление доступом для кода является устаревшим, большинство современных кодов в экосистеме .NET не предназначены для использования в среде с частичным доверием. Приложения, зависящие от CAS, не должны предполагать, что современные библиотеки будут правильно работать в этих средах, если разработчик библиотеки не предполагают явные действия по обеспечению совместимости в песочнице CAS.  
