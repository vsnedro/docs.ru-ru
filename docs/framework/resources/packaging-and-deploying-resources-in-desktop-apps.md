...
> [!NOTE]
> Есть некоторые исключения для двухбуквенные названия `zh-Hans` культуры, такие как для китайского (упрощенный).
...
> [!TIP]
> Возможно, вы сможете использовать [ \<элемент конфигурации relativeBindForResources>](../configure-apps/file-schema/runtime/relativebindforresources-element.md) для оптимизации процесса резервного копирования ресурсов и процесса, с помощью которого зондирует время выполнения для сборки ресурсов. Дополнительные сведения см. в разделе [Оптимизация процесса использования резервных ресурсов](packaging-and-deploying-resources-in-desktop-apps.md#Optimizing).
...
Оптимизируйте зонд для спутниковых сборок, включив [ \<элемент relativeBindForResources>](../configure-apps/file-schema/runtime/relativebindforresources-element.md) элемент и установив его `enabled` атрибут `true` в файл конфигурации приложения, как показано в следующем примере.
...
- [Ресурсы в настольных приложениях](index.md)
- [Кэш Глобальной Ассамблеи](../app-domains/gac.md)
