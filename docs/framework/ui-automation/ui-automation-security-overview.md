...
В этом обзоре описывается модель безопасности [!INCLUDE[TLA#tla_uiautomation](../../../includes/tlasharptla-uiautomation-md.md)] в Windows Vista.
...
Чтобы получить доступ к защищенному системному ИНТЕРФЕЙСу, приложения должны быть построены с помощью файла манифеста, который включает `uiAccess` атрибут в `requestedExecutionLevel` тег следующим образом:
