...
> [!NOTE]
> Используйте [служебную программу метаданных ServiceModel (Svcutil.exe)](servicemodel-metadata-utility-tool-svcutil-exe.md) с `/config` параметром для быстрого создания файлов конфигурации.  
...
### <a name="servicemodel-elements"></a>Элементы ServiceModel  
 ...
 Например, указание [\<basicHttpBinding>](../configure-apps/file-schema/wcf/basichttpbinding.md) элемента указывает на использование HTTP в качестве транспорта для конечной точки. Это используется для подключения конечной точки во время выполнения, когда открыта служба, использующая данную конечную точку.  
...
## <a name="specifying-a-service-with-two-endpoints-using-different-binding-values"></a>Указание службы с двумя конечными точками при помощи различных значений привязки  
 В последнем примере две конечные точки настраиваются для типа службы `HelloWorld`. Каждая конечная точка использует другой настраиваемый `bindingConfiguration` атрибут того же типа привязки (каждый из них изменяет `basicHttpBinding` ).  
