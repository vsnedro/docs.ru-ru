...
description: Сведения о том, как модель контрактов данных обрабатывает коллекции в .NET Framework и как WCF поддерживает сериализацию данных для типов коллекций.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
ms.locfileid: "85245184"
---
# <a name="collection-types-in-data-contracts"></a>Типы коллекций в контрактах данных

Под *коллекцией* понимается список элементов определенного типа. В .NET Framework такие списки можно представить с помощью массивов или различных типов (универсальный список, универсальный тип <xref:System.ComponentModel.BindingList%601> , <xref:System.Collections.Specialized.StringCollection> или <xref:System.Collections.ArrayList> ). Например, в коллекции может содержаться список адресов конкретного клиента. Такие коллекции называются *коллекциями списков*, независимо от их фактического типа.
...
Типы, содержащиеся в коллекциях, должны быть контрактными данными, либо сериализуемые другим способом. Дополнительные сведения см. [в разделе Типы, поддерживаемые сериализатором контрактов данных](types-supported-by-the-data-contract-serializer.md).
...
Во время десериализации, если объявленный тип является интерфейсом, ядро сериализации выбирает тип, который реализует объявленный интерфейс, и создается экземпляр типа. Механизм известных типов (описанный в разделе « [известные типы контрактов данных](data-contract-known-types.md)») не оказывает никакого влияния; выбранный тип встроен в WCF.
...
Правила именования типов коллекций подобны правилам именования обычных типов контракта данных, как описано в разделе [Data Contract Names](data-contract-names.md). Однако есть некоторые различия.
...
Коллекции словарей помечены заметкой схемы, зависящей от WCF, которая указывает, что они являются словарями; в противном случае они становятся неразличимыми из простых списков, содержащих записи с ключом и значением. Подробное описание представлений коллекций в схеме контракта данных см. в статье [Data Contract Schema Reference](data-contract-schema-reference.md).
...
Тем не менее, для настроенных коллекций создаются отдельные типы, отмеченные атрибутом <xref:System.Runtime.Serialization.CollectionDataContractAttribute> . (В схеме настроенный тип коллекции не использует пространство имен по умолчанию, имя, имя повторяющегося элемента, а также имена элементов ключ/значение.) Эти типы являются пустыми типами, производными от универсальных <xref:System.Collections.Generic.List%601> типов списков и универсальных словарей для типов словаря.
...
\*<xref:System.Runtime.Serialization.NetDataContractSerializer>В этом случае используется настройка класса. В данном случае класс <xref:System.Runtime.Serialization.NetDataContractSerializer> также сериализует текущее имя типа, т. е. десериализация выполняется так, как ожидалось.
