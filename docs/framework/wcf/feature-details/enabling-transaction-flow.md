...
|Допускается|Да|WS-AT|Транзакция может передаваться в обеспечивающем возможность взаимодействия формате WS-AT.|  
|Допускается|Да|OleTransactions|Транзакция может передаваться в формате WCF OleTransactions.|  
|Допускается|false|Любое значение|Транзакция не передается.|  
...
|Транзакция не соответствует ожидаемому формату протокола|Допускается|`MustUnderstand` равняется `false`.|Отклонена, так как заголовок не распознан|  
...
|Транзакция отсутствует|Допускается|н/д|Процесс|  
