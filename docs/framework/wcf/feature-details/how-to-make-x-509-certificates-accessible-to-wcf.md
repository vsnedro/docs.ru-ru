...
1. Предоставьте учетной записи, под которой выполняется служба WCF, доступ на чтение к файлу, содержащему закрытый ключ, связанный с сертификатом X. 509.  
    ...
    3. Определите, где закрытый ключ сертификата находится на компьютере с помощью средства [FindPrivateKey](../samples/findprivatekey.md) .  
         ...
         В следующем примере кода используется средство [FindPrivateKey](../samples/findprivatekey.md) , чтобы определить расположение закрытого ключа для сертификата в `My` хранилище в `CurrentUser` с отпечатком `46 dd 0e 7a ed 0b 7a 31 9b 02 a3 a0 43 7a d8 3f 60 40 92 9d` .  
