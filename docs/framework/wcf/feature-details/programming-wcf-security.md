...
ms.locfileid: "81463822"
---
# <a name="programming-wcf-security"></a>Программирование безопасности WCF
Эта тема описывает основные задачи программирования, используемые для создания безопасного приложения Windows Communication Foundation (WCF). Эта тема охватывает только аутентификацию, конфиденциальность и целостность, известную как *безопасность передачи.* Эта тема не охватывает авторизацию (контроль доступа к ресурсам или услугам); для получения информации о разрешении, [см.](../../../../docs/framework/wcf/feature-details/authorization-in-wcf.md)  
  
> [!NOTE]
> Для ценного введения в концепции безопасности, особенно в отношении WCF, см. набор шаблонов и практики учебники по MSDN в [сценариях, шаблонах и руководстве по реализации для улучшения веб-служб (WSE) 3.0](https://docs.microsoft.com/previous-versions/msp-n-p/ff648183(v=pandp.10)).  
  
 Программирование безопасности WCF основано на трех шагах, определяющих следующие: режим безопасности, тип учетных данных клиента и значения учетных данных. Эти действия можно выполнить с помощью кода или конфигурации.  
  
## <a name="setting-the-security-mode"></a>Задание режима безопасности  
 Ниже приводятся общие шаги для программирования с режимом безопасности в WCF:  
  
1. Выберите одну из предопределенных привязок, отвечающих требованиям приложения. Список обязательных вариантов можно узнать в [системе.](../../../../docs/framework/wcf/system-provided-bindings.md) По умолчанию практически во всех привязках включены функции безопасности. Единственным исключением <xref:System.ServiceModel.BasicHttpBinding> является класс (с использованием конфигурации, [ \<basicHttpBinding>). ](../../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)  
...
2. Выберите один из режимов безопасности привязки. Обратите внимание, что выбранная привязка определяет выбор доступных режимов. Например, привязка <xref:System.ServiceModel.WSDualHttpBinding> не позволяет обеспечить безопасность транспорта (это изменить нельзя). Аналогично, привязки <xref:System.ServiceModel.MsmqIntegration.MsmqIntegrationBinding> и <xref:System.ServiceModel.NetNamedPipeBinding> не обеспечивают безопасность сообщений.  
    ...
    2. `Message`  
         ...
         Для получения дополнительной [информации см.](../../../../docs/framework/wcf/feature-details/message-security-in-wcf.md)  
...
3. Если принято решение использовать безопасность транспорта для протокола HTTP (т.е. протокол HTTPS), необходимо настроить узел с сертификатом SSL и включить SSL для порта. Для получения дополнительной [информации см.](../../../../docs/framework/wcf/feature-details/http-transport-security.md)  
...
## <a name="setting-the-client-credential-type"></a>Задание типа учетных данных клиента  
 Выберите необходимый тип учетных данных клиента. Для получения дополнительной [информации см.](../../../../docs/framework/wcf/feature-details/selecting-a-credential-type.md) Доступны следующие типы учетных данных клиента:  
