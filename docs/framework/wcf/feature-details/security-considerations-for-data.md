...
ms.locfileid: "86441821"
---
# <a name="security-considerations-for-data"></a>Вопросы безопасности для данных

При работе с данными в Windows Communication Foundation (WCF) необходимо учитывать ряд категорий угроз. В следующей таблице представлен список наиболее важных классов угроз, относящихся к обработке данных. WCF предоставляет средства для устранения этих угроз.

Отказ в обслуживании при получении ненадежных данных данные могут привести к тому, что принимающая сторона сможет получить доступ к непропорциональному объему различных ресурсов, таких как память, потоки, доступные подключения или циклы процессора, вызывая длительные вычисления. Атака типа "отказ в обслуживании" в отношении сервера может стать причиной его сбоя и невозможности обработки сообщений от других, допустимых клиентов.

При выполнении вредоносного кода входящие ненадежные данные приводят к тому, что принимающая сторона выполняет код, который им не планировалось.

Раскрытие информации. удаленный злоумышленник заставляет принимающую сторону отвечать на свои запросы таким образом, чтобы раскрывать больше информации, чем это происходит.
