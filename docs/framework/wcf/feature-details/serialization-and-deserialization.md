...
* Имеет уровень ***не** _ Secure. Дополнительные сведения см. в разделе [BinaryFormatter Security Guide](../../../standard/serialization/binaryformatter-security-guide.md).
Параметр _ аналогичен <xref:System.Runtime.Serialization.Formatters.Binary.BinaryFormatter> <xref:System.Runtime.Serialization.Formatters.Soap.SoapFormatter> сериализаторам и, поскольку он также выдает имена типов .NET Framework как часть сериализованных данных.
...
## <a name="using-the-netdatacontractserializer"></a>Использование NetDataContractSerializer  
 ...
 Основное различие между `DataContractSerializer` и состоит в том <xref:System.Runtime.Serialization.NetDataContractSerializer> , что `DataContractSerializer` использует имена контрактов данных, а `NetDataContractSerializer` выходные данные полны .NET Framework сборки и имена типов в сериализованном XML. Это означает, что одни и те же типы должны совместно использоваться конечными точками сериализации и десериализации. Так как при использовании сериализатора `NetDataContractSerializer` всегда известны точные типы, которые должны быть десериализованы, механизм известных типов не требуется.  
...
- Производительность. Запись имен типов и сборок значительно увеличивает размер получаемого XML-кода.  
 ...
 Этот механизм похож на двоичную или SOAP-сериализацию, используемую .NET Framework удаленным взаимодействием (в частности, <xref:System.Runtime.Serialization.Formatters.Binary.BinaryFormatter> и <xref:System.Runtime.Serialization.Formatters.Soap.SoapFormatter> ).  
