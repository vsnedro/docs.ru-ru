...
## <a name="msmq-addressing"></a>Адресация к очереди сообщений MSMQ  
 ...
 Дополнительные сведения о пути MSMQ и именах форматов см. в статье [об очередях сообщений](/previous-versions/windows/desktop/legacy/ms706032(v=vs.85)).  
...
- \<*queue-name*> имя очереди. Имя очереди также может относиться к вложенной очереди. Таким таким, \<*queue-name*>  =  \<*name-of-queue*> [;* вложенное имя очереди*].  
 ...
 Конечная точка, выполняющая чтение из очереди, должна указывать адрес очереди с помощью указанной выше схемы при открывании узла службы (ServiceHost). Примеры см. в разделе [Привязка к сети MSMQ](../samples/net-msmq-binding.md).  
...
## <a name="msmqintegrationbinding-and-service-addressing"></a>MsmqIntegrationBinding и адресация к службе  
 ...
 Имя MSMQ-format-name указывается в формате MSMQ в статье [об очереди сообщений](/previous-versions/windows/desktop/legacy/ms706032(v=vs.85)).  
