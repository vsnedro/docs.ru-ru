...
```  
 ...
 При размещении службы WCF в службах IIS IIS создает по одному базовому адресу на основе URI виртуального каталога, содержащего приложение. Чтобы добавить одну или несколько привязок, можно с помощью диспетчера служб IIS добавить на веб-сайт дополнительные базовые адреса, использующие тот же протокол. Для каждой привязки укажите протокол (HTTP или HTTPS), IP-адрес, порт и имя узла. Дополнительные сведения об использовании диспетчера службы IIS см. в разделе [IIS Manager (IIS 7)](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753842(v=ws.10)). Дополнительные сведения о добавлении привязок на сайт см. в разделе [Создание веб-сайта (IIS 7)](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc772350(v=ws.10)) .  
