...
А) Проверьте контракт службы. Возможно, вы забыли разместить "IsOneWay = `true` " во всех операциях службы. Очереди поддерживают только односторонние операции службы.
