...
- При импорте схем `DataSet` или `DataTable` важно сделать ссылку на эти типы. С помощью программы командной строки Svcutil.exe это можно сделать, передав имя System.Data.dllной сборки в `/reference` коммутатор. Если импортируется схема типизированного набора данных, необходимо создать ссылку на тип типизированного набора данных. При использовании Svcutil.exe передайте расположение сборки типизированного набора данных в `/reference` параметр. Дополнительные сведения о ссылочных типах см. в разделе [Импорт схемы для создания классов](importing-schema-to-generate-classes.md).  
