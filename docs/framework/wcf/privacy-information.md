...
#### <a name="information-removed-from-message-headers-when-logging-decryptedunencrypted-messages"></a>Информация, удаленная из заголовков сообщений при внесении в журнал зашифрованных/незашифрованных сообщений  
 ...
 \<AssertionIDReference>УДОСТОВЕРЕНИЯ\</AssertionIDReference>*  
 ...
 Решение = "[разрешить&#124;запретить&#124;неопределенного]"  
...
### <a name="other-features-for-it-professionals"></a>Другие функции для ИТ-специалистов  
 ...
 В конфигурации WCF используется механизм настройки платформа .NET Framework. Файлы конфигурации хранятся на компьютере. Разработчик приложения и администратор создают файлы конфигурации и элемент правления доступом для каждого требования приложения. Файл конфигурации может содержать адреса конечных точек и ссылки на сертификаты в хранилище сертификатов. Сертификаты можно использовать, чтобы предоставить данные приложения для настройки различных свойств возможностей, используемых приложением.  
  
 WCF также использует функцию дампа процесса платформа .NET Framework, вызывая <xref:System.Environment.FailFast%2A> метод.  
...
#### <a name="wsatconfigexe-and-wsatuidll"></a>WSATConfig.exe и WSATUI.dll  
 ...
 Эти средства позволяют ИТ-специалистам настраивать взаимодействующую WS-AtomicTransactionную поддержку сети в WCF. Средство отображает и позволяет пользователю изменять значения наиболее часто используемых параметров WS-AtomicTransaction, хранящихся в реестре.  
