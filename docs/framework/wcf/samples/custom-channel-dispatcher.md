...
## <a name="creating-a-response-message"></a>Создание ответного сообщения  
 ...
 Обработка сообщений реализуется в типе `MyServiceManager`. В методе `HandleRequest` сначала проверяется заголовок `Action` сообщения, чтобы выяснить, поддерживается ли запрос. http://tempuri.org/HelloWorld/HelloДля обеспечения фильтрации сообщений определено предопределенное действие SOAP "". Это похоже на концепцию контракта службы в реализации WCF <xref:System.ServiceModel.ServiceHost> .  
 ...
 Клиент этого образца является обычным клиентом WCF, который не предполагает никаких действий от службы. Таким образом, служба специально разработана для соответствия тем, что вы получаете из обычной <xref:System.ServiceModel.ServiceHost> реализации WCF. В результате этого в клиенте требуется наличие только контракта службы.  
