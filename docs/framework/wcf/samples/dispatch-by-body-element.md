...
## <a name="implementing-the-service"></a>Реализация службы  
 ...
 Пример службы проекта применяет `DispatchByBodyElementBehaviorAttribute` поведение контракта к `IDispatchedByBody` контракту службы и помечает каждую из этих операций `OperationForBodyA()` и `OperationForBodyB()` `DispatchBodyElementAttribute` поведение операции. Когда открыт узел службы, который реализует этот контракт, эти метаданные используются конструктором диспетчера в соответствии с приведенным описанием.  
