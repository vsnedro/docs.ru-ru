...
## <a name="pii-logging-configuration"></a>Конфигурация регистрации персональных данных (PII)  
 ...
 Параметр на уровне компьютера управляется путем установки значения `enableLoggingKnownPii` `true` или `false` в `machineSettings` элементе в файле Machine. config. Например, следующий пример позволяет приложениям включить ведение журнала личных данных.  
