...
- Конечная точка ~/service.svc/soap позволяет клиентам получать доступ к службе с использованием привязки протокола SOAP.  
 ...
 Конечная точка HTTP настроена с помощью <`webHttp`> стандартной конечной точки, `helpEnabled` для которой задано значение `true` . В результате этого служба предоставляет справочную страницу на основе XHTML по адресу ~/service.svc/http/help, которую клиенты на основе HTTP могут использовать для доступа к службе.  
