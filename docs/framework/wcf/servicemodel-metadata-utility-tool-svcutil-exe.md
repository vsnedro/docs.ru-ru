...
Программа не поддерживает настройку для различных версий. Если вы хотите создать артефакт .NET 4 из *svcutil.exe*, используйте *svcutil.exe* из пакета SDK для .NET 4. Чтобы создать артефакт .NET 3.5, используйте исполняемый файл из пакета SDK платформы .NET 3.5.
