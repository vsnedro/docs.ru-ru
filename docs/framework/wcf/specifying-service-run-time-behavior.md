...
ms.locfileid: "81243106"
---
# <a name="specifying-service-run-time-behavior"></a>Указание поведения службы во время выполнения
После создания ([Designing Service Contracts](designing-service-contracts.md)) и реализации ([Implementing Service Contracts](implementing-service-contracts.md)) контракта службы можно переходить к настройке рабочего поведения для среды выполнения службы. В этом разделе рассматриваются предоставляемые системой поведения служб и операций, а также указывается, где можно найти дополнительные сведения по созданию новых поведений. Хотя некоторые поведения применяются как атрибуты, многие поведения применяются с помощью файла конфигурации приложения или программно. Для получения дополнительной информации о настройке приложения службы [см.](configuring-services.md)  
...
## <a name="overview"></a>Обзор  
 ...
 Однако конкретные сведения об операциях, такие как сведения о потоках или управлении экземплярами, не видны клиентам. После реализации контракта службы можно настроить большое количество характеристик операций с помощью *поведений*. Поведение — это объекты, которые изменяют время выполнения Windows Communication Foundation (WCF), установив свойство времени выполнения или вставив тип настройки в время выполнения. Для получения дополнительной информации об изменении времени выполнения путем создания поведения, определяемого пользователем, [см.](./extending/extending-servicehost-and-the-service-model-layer.md)  
 ...
 Эта тема содержит обзор <xref:System.ServiceModel.ServiceBehaviorAttribute> и <xref:System.ServiceModel.OperationBehaviorAttribute> атрибуты, описывает различные области, в которых может работать поведение, и дает краткое описание многих системных моделей поведения в различных областях, которые могут представлять интерес для разработчиков WCF.  
...
- <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsAttribute>. Применяется к службе WCF, чтобы указать, можно ли запустить эту службу в режиме ASP.NET совместимости.  
...
- <xref:System.ServiceModel.Description.ServiceDebugBehavior>. Обеспечивает отладку и помощь в предоставлении информации для службы WCF.  
...
- <xref:System.ServiceModel.Description.CallbackDebugBehavior>. Позволяет отлавливание службы для объекта обратного вызова WCF.  
...
- <xref:System.ServiceModel.Description.MustUnderstandBehavior>. Поручает WCF отключить `MustUnderstand` обработку.  
