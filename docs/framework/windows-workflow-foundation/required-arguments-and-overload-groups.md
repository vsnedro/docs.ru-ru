...
```  
 ...
 В следующем примере, взятом из примера [действий доступа к базе данных](./samples/database-access-activities.md) , есть две группы перегруженных версий: `ConnectionString` и `ConfigFileSectionName` . Чтобы данное действие было допустимым, должны быть связаны либо аргументы `ProviderName` и `ConnectionString`, либо аргумент `ConfigName`, но не оба.  
