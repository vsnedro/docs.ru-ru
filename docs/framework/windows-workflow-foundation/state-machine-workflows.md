...
## <a name="creating-and-configuring-transitions"></a>Создание и настройка переходов  
 ...
 Все состояния должны иметь по крайней мере один переход, за исключением окончательного состояния, в котором могут отсутствовать переходы. Переходы можно добавлять после добавления состояния в рабочий процесс конечного автомата, а также при сбросе состояния в область конструктора.  
...
> Для одного состояния в конечном автомате может быть создано до 76 переходов в конструкторе рабочих процессов. Количество переходов состояний для рабочих процессов, созданных за пределами конструктора, ограничивается только системными ресурсами.  
 ...
 Переход может содержать объекты <xref:System.Activities.Statements.Transition.Trigger%2A>, <xref:System.Activities.Statements.Transition.Condition%2A> и <xref:System.Activities.Statements.Transition.Action%2A>. Переход выполняется по <xref:System.Activities.Statements.Transition.Trigger%2A> расписанию при завершении действия состояния источника перехода <xref:System.Activities.Statements.State.Entry%2A> . Обычно <xref:System.Activities.Statements.Transition.Trigger%2A> представляет собой действие, ожидающее появления определенного типа события, но он может быть любым действием или даже не иметь действия. После завершения действия <xref:System.Activities.Statements.Transition.Trigger%2A> вычисляется значение <xref:System.Activities.Statements.Transition.Condition%2A> (если существует). Если <xref:System.Activities.Statements.Transition.Trigger%2A> действий нет, <xref:System.Activities.Statements.Transition.Condition%2A> немедленно оценивается. Если условие принимает значение `false` , переход отменяется, а <xref:System.Activities.Statements.Transition.Trigger%2A> действие для всех переходов из состояния Перепланируется. Если существуют другие переходы, имеющие то же состояние источника, что и текущий переход, эти <xref:System.Activities.Statements.Transition.Trigger%2A> действия отменяются и перепланируются. Если <xref:System.Activities.Statements.Transition.Condition%2A> имеет значение `true` или условие отсутствует, выполняется действие <xref:System.Activities.Statements.State.Exit%2A> исходного состояния, а затем выполняется <xref:System.Activities.Statements.Transition.Action%2A> перехода. По <xref:System.Activities.Statements.Transition.Action%2A> завершении управление передается в **целевое** состояние  
...
## <a name="state-machine-terminology"></a>Терминология конечного автомата  
 ...
 Переход  
 Направленная связь между двумя состояниями, представляющая полный ответ конечного автомата на вхождение события определенного типа.  
 ...
 Условие  
 Ограничение, которое должно возвращаться `true` после наступления триггера, чтобы завершить переход.  
 ...
 Переход на себя  
 Переход, который передается от состояния к самому себе.  
  
 Начальное состояние  
 Состояние, представляющее начальную точку конечного автомата.  
