...
### <a name="identifier-name-is-not-a-valid-identifier"></a>" \<identifier name> " не является допустимым идентификатором

Эта ошибка указывает на неправильное именование поля, метода, события или объекта.

### <a name="name-already-exists-in-project-name"></a>" \<name> " уже существует в " \<project name> "

Сообщение об ошибке: "" \<name> уже существует в " \<project name> ". Введите уникальное имя ".
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>" \<Toolbox tab name> " не является категорией панели элементов
...
### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Произошло исключение при попытке создать экземпляр " \<object name> "

Сообщение об ошибке: "произошло исключение при попытке создать экземпляр" \<object name> ". Исключением было " \<exception string\> ".
...
### <a name="another-editor-has-document-name-open-in-an-incompatible-mode"></a>Другой редактор " \<document name> " открыт в несовместимом режиме

Сообщение об ошибке: "другой редактор \<document name> открыт в несовместимом режиме" ". Закройте редактор и повторите операцию ".
...
Это сообщение похоже на «другой редактор \<document name> , Открытый в несовместимом режиме», но Visual Studio не удается определить имя файла. Чтобы исправить эту ошибку, закройте редактор, в котором открыт файл, и повторите попытку.

### <a name="array-rank-rank-in-array-is-too-high"></a>Ранг массива " \<rank in array> " слишком велик
...
### <a name="assembly-assembly-name-could-not-be-opened"></a>\<assembly name>Не удалось открыть сборку ""

Сообщение об ошибке: "сборка" \<assembly name> не может быть открыта. Убедитесь, что файл по-прежнему существует. "
...
### <a name="bad-element-type-this-serializer-expects-an-element-of-type-type-name"></a>Недопустимый тип элемента. Этот сериализатор принимает элемент типа " \<type name> "
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Не удается привязать обработчик событий к \<event name> событию "", так как оно доступно только для чтения
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Невозможно присвоить имя объекту " \<name> ", так как он уже называется " \<name> "
...
### <a name="cannot-remove-or-destroy-inherited-component-component-name"></a>Невозможно удалить или уничтожить унаследованный компонент " \<component name> "
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>Категория " \<Toolbox tab name> " не содержит средство для класса " \<class name> "
...
### <a name="code-generation-for-property-property-name-failed"></a>Не удалось создать код для свойства " \<property name> "
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Компонент " \<component name> " не вызывал контейнер. Add () в его конструкторе
...
### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Не удалось получить доступ к переменной " \<variable name> ", так как она еще не инициализирована
...
### <a name="could-not-find-type-type-name"></a>Не удалось найти тип " \<type name> "

Сообщение об ошибке: "не удалось найти тип" \<type name> ". Убедитесь, что ссылка на сборку, содержащую этот тип, указана. Если этот тип является частью проекта разработки, убедитесь, что проект успешно построен. "
...
### <a name="could-not-load-type-type-name"></a>Не удалось загрузить тип " \<type name> "

Сообщение об ошибке: "не удалось загрузить тип" \<type name> ". Убедитесь, что сборка, содержащая этот тип, добавлена в ссылки проекта ".
...
### <a name="delegate-class-class-name-has-no-invoke-method-is-this-class-a-delegate"></a>Класс делегата " \<class name> " не имеет метода Invoke. Является ли этот класс делегатом?
...
### <a name="duplicate-declaration-of-member-member-name"></a>Повторяющееся объявление члена " \<member name> "
...
### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Ошибка при чтении ресурсов из файла ресурсов для языка и региональных параметров " \<culture name> "
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Ошибка при чтении ресурсов из файла ресурсов для языка и региональных параметров по умолчанию " \<culture name> "
...
### <a name="failed-to-parse-method-method-name"></a>Не удалось проанализировать метод " \<method name> "

Сообщение об ошибке: "не удалось проанализировать метод" \<method name> ". Средство синтаксического анализа сообщило о следующей ошибке: " \<error string> ". Возможные ошибки можно найти в список задач ".
...
### <a name="invalid-component-name-component-name"></a>Недопустимое имя компонента: " \<component name> "
...
### <a name="the-type-class-name-is-made-of-several-partial-classes-in-the-same-file"></a>Тип " \<class name> " состоит из нескольких разделяемых классов в одном файле
...
### <a name="the-assembly-assembly-name-could-not-be-found"></a>\<assembly name>Не удалось найти сборку ""

Сообщение об ошибке: "сборка" \<assembly name> не найдена. Убедитесь, что ссылка на сборку указана. Если сборка является частью текущего проекта разработки, убедитесь, что проект построен. "

Эта ошибка похожа на "тип" \<type name> не найден ", но эта ошибка обычно возникает из-за атрибута метаданных. Чтобы исправить эту ошибку, убедитесь, что имеются ссылки на все сборки, используемые атрибутами.

### <a name="the-assembly-name-assembly-name-is-invalid"></a>Недопустимое имя сборки " \<assembly name> "
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>Базовый класс " \<class name> " не может быть спроектирован
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>\<class name>Не удалось загрузить базовый класс ""
...
### <a name="the-class-class-name-cannot-be-designed-in-this-version-of-visual-studio"></a>Класс " \<class name> " не может быть спроектирован в этой версии Visual Studio
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Не удается добавить компонент, так как он содержит циклическую ссылку на " \<reference name> "
...
### <a name="the-designer-for-base-class-class-name-is-not-installed"></a>Конструктор для базового класса " \<class name> " не установлен
...
### <a name="the-designer-must-create-an-instance-of-type-type-name-but-it-cant-because-the-type-is-declared-as-abstract"></a>Конструктор должен создать экземпляр типа " \<type name> ", но это не может быть вызвано тем, что тип объявлен как абстрактный
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>Класс анализатора языка " \<class name> " не реализован должным образом

Сообщение об ошибке: "класс средства синтаксического анализа языка" \<class name> "не реализован должным образом. Обратитесь к поставщику за обновленным модулем синтаксического анализатора.
...
### <a name="the-name-name-is-already-used-by-another-object"></a>Имя " \<name> " уже используется другим объектом
...
### <a name="the-object-object-name-does-not-implement-the-icomponent-interface"></a>Объект " \<object name> " не реализует интерфейс IComponent
...
### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>Объект " \<object name> " вернул значение NULL для свойства " \<property name> ", но это не допускается
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Служба " \<service name> " является обязательной, но ее не удалось найти

Сообщение об ошибке: "служба" \<service name> является обязательной, но ее не удалось найти. Возможно, возникла проблема с установкой Visual Studio ".
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>Не удалось получить элемент панели элементов для " \<component name> " из панели элементов

Сообщение об ошибке: "не удалось получить элемент панели элементов для" \<component name> "из панели элементов. Убедитесь, что сборка, содержащая элемент панели элементов, установлена правильно. Элемент панели элементов вызвал следующую ошибку: \<error string> "

Рассматриваемый компонент вызвал исключение при обращении Visual Studio к нему. Обратитесь к поставщику компонента.

### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Не удалось получить элемент панели элементов для " \<Toolbox item name> " из панели элементов

Сообщение об ошибке: "не удалось получить элемент панели элементов для" \<Toolbox item name> "из панели элементов. Попробуйте удалить элемент из панели элементов и добавить его обратно. "
...
### <a name="the-type-type-name-could-not-be-found"></a>\<type name>Не удалось найти тип ""

Сообщение об ошибке: " \<type name> не удалось найти тип" ". Убедитесь, что ссылка на сборку, содержащую тип, указана. Если сборка является частью текущего проекта разработки, убедитесь, что проект построен. "
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>Переменная " \<variable name> " либо не объявлена, либо не была назначена
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Обработчик команд для команды меню "" уже существует. \<menu command name>
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Для формата "" уже зарегистрирован создатель элемента панели элементов \<format name>
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Тип " \<type name\> " не имеет конструктора с параметрами типов " \<parameter type names> "
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Не удалось добавить ссылку " \<reference name> " в текущее приложение

Сообщение об ошибке: "не удается добавить ссылку" \<reference name> "в текущее приложение. Убедитесь, что на другую версию " \<reference name> " еще не ссылается ".
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Не удается найти страницу с именем " \<Options dialog box tab name> "
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Не удается найти свойство " \<property name> " на странице " \<Options dialog box tab name> "
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Visual Studio не может сохранить или загрузить экземпляры типа " \<type name> "

Это проблема с компонентом стороннего производителя. Обратитесь к поставщику компонента.

### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Visual Studio не удается открыть " \<document name> " в представление конструирования

Сообщение об ошибке: "Visual Studio не удается открыть" \<document name> "в представление конструирования. Средство синтаксического анализа не установлено для типа файлов. "
...
### <a name="visual-studio-was-unable-to-find-a-designer-for-classes-of-type-type-name"></a>Visual Studio не удалось найти конструктор для классов типа " \<type name> "
