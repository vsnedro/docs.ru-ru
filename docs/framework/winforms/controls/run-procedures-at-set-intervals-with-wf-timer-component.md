...
## <a name="example"></a>Пример  
 Во втором примере кода процедура выполняется каждые 600 миллисекунд до завершения цикла. В следующем примере кода предполагается, что у вас есть форма с <xref:System.Windows.Forms.Button> элементом управления с именем `Button1` , <xref:System.Windows.Forms.Timer> элементом управления с именем `Timer1` и <xref:System.Windows.Forms.Label> элементом управления с именем `Label1` .  
