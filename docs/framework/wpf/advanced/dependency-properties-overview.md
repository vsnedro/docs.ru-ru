...
В следующем примере создается простой стиль (который будет определен в <xref:System.Windows.FrameworkElement.Resources%2A> словаре, не показан), а затем применяет этот стиль непосредственно к <xref:System.Windows.FrameworkElement.Style%2A> свойству для <xref:System.Windows.Controls.Button> . Метод задания в стиле задает для свойства <xref:System.Windows.Controls.Control.Background%2A> элемента управления <xref:System.Windows.Controls.Button> значение "зеленый".
...
Как правило, стили не должны всегда применяться и скрывать даже локально заданное значение отдельного элемента (в противном случае было бы сложно использовать либо стили, либо элементы в целом). Поэтому значения, определяемые стилем, имеют более низкий приоритет, чем локально заданные значения. Более полный список свойств зависимостей и порядок определения их действующих значений см. в разделе [Приоритет значений свойств зависимостей](dependency-property-value-precedence.md).
...
- Рассмотрим свойства зависимостей как открытые свойства, доступные или по крайней мере обнаруживаемые любым вызывающим объектом, имеющим доступ к экземпляру. Дополнительные сведения см. в разделе [Безопасность свойств зависимостей](dependency-property-security.md).
