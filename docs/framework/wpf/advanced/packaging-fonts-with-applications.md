...
### <a name="enumerating-fonts-in-an-application"></a>Перечисление шрифтов в приложении  
 ...
 В следующем примере показано, как использовать <xref:System.Windows.Media.Fonts.GetTypefaces%2A> метод для возврата коллекции <xref:System.Windows.Media.Typeface> объектов из расположения шрифта приложения. В данном случае приложение содержит подкаталог с именем resources.  
