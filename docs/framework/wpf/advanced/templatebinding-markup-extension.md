...
## <a name="remarks"></a>Remarks  
 — Это оптимизированная форма [привязки](binding-markup-extension.md) для сценариев шаблонов, `Binding` аналогичная построению с помощью `{Binding RelativeSource={RelativeSource TemplatedParent}, Mode=OneWay}` `TemplateBinding` `TemplateBinding` всегда является односторонней привязкой, даже если используемые свойства по умолчанию используют двустороннюю привязку. Оба используемых свойства должны быть свойствами зависимостей. Чтобы обеспечить двустороннюю привязку к шаблонному родителю, используйте следующую инструкцию привязки `{Binding RelativeSource={RelativeSource TemplatedParent}, Mode=TwoWay, Path=MyDependencyProperty}`.
  
 [RelativeSource](relativesource-markupextension.md) — это еще одно расширение разметки, которое иногда используется вместе с или `TemplateBinding` вместо для выполнения относительной привязки свойства в шаблоне.  
...
```  
 ...
 В реализации [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] процессора XAML обработка этого расширения разметки определяется <xref:System.Windows.TemplateBindingExtension> классом.  
