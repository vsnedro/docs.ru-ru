...
description: Узнайте больше о присутствии и назначении двух сопоставлений пространств имен XAML, часто обнаруженных в корневом теге файла Windows Presentation Foundation XAML.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
## <a name="wpf-and-assembly-loading"></a>WPF и загрузка сборок  
 Контекст схемы XAML для WPF интегрируется с моделью приложения WPF, которая, в свою очередь, использует концепцию, определяемую CLR <xref:System.AppDomain> . В следующей последовательности описывается, как контекст схемы XAML интерпретирует, как загружать сборки или находить типы во время выполнения или во время разработки, основываясь на использовании WPF <xref:System.AppDomain> и других факторах.  
  
1. Итерация по <xref:System.AppDomain> , поиск уже загруженной сборки, соответствующей всем аспектам имени, начиная с последней загруженной сборки.  
...
4. Для вызова используйте короткое имя + токен открытого ключа <xref:System.Reflection.Assembly.Load%28System.String%29?displayProperty=nameWithType> .  
