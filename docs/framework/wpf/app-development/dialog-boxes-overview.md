...
## <a name="message-boxes"></a>Окна сообщений  
 ...
 Дополнительные сведения об использовании окон сообщений см. в разделе <xref:System.Windows.MessageBox> , [образец MessageBox](https://github.com/Microsoft/WPF-Samples/tree/master/Windows/MessageBox)и [Образец диалогового окна](https://github.com/Microsoft/WPF-Samples/tree/master/Windows/DialogBox).  
...
### <a name="open-file-dialog"></a>Диалоговое окно открытия файла  
 ...
 Общее диалоговое окно открытия файла реализуется как <xref:Microsoft.Win32.OpenFileDialog> класс и находится в <xref:Microsoft.Win32> пространстве имен. Следующий код показывает, как создавать, настраивать и отображать такое окно, а также как обрабатывать результат.  
...
### <a name="save-file-dialog-box"></a>диалоговое окно сохранения файлов  
 ...
 Диалоговое окно «Общие файлы сохранения» реализуется как <xref:Microsoft.Win32.SaveFileDialog> класс и находится в <xref:Microsoft.Win32> пространстве имен. Следующий код показывает, как создавать, настраивать и отображать такое окно, а также как обрабатывать результат.  
...
Общее диалоговое окно печати реализуется как <xref:System.Windows.Controls.PrintDialog> класс и находится в <xref:System.Windows.Controls> пространстве имен. Следующий код показывает, как создавать, настраивать и отображать такое окно.  
...
- Клавиша ESC в качестве сочетания клавиш, которое вызывает нажатие кнопки **Отмена** . Это можно сделать, задав <xref:System.Windows.Controls.Button.IsCancel%2A> для свойства кнопки **Отмена** значение `true` .  
  
- Клавиша ВВОД (или возврата) в качестве сочетания клавиш, которое вызывает нажатие кнопки **ОК** . Для этого нужно задать <xref:System.Windows.Controls.Button.IsDefault%2A> свойство кнопки **ОК** `true` .  
...
Чтобы проверить привязанный элемент управления в WPF, необходимо определить правило проверки и связать его с привязкой. Правило проверки является пользовательским классом, производным от <xref:System.Windows.Controls.ValidationRule> . В следующем примере показано правило проверки, `MarginValidationRule` которое проверяет, что привязанное значение — <xref:System.Double> и находится в указанном диапазоне.  

[!code-csharp[Margin validation rules](~/samples/snippets/csharp/VS_Snippets_Wpf/DialogBoxSample/CSharp/MarginValidationRule.cs)]
[!code-vb[Margin validation rules](~/samples/snippets/visualbasic/VS_Snippets_Wpf/DialogBoxSample/VisualBasic/MarginValidationRule.vb)]  

В этом коде логика проверки правила проверки реализуется путем переопределения <xref:System.Windows.Controls.ValidationRule.Validate%2A> метода, который проверяет данные и возвращает соответствующее значение <xref:System.Windows.Controls.ValidationResult> .  
...
Открытие диалогового окна с помощью <xref:System.Windows.Window.ShowDialog%2A> является фундаментальным методом, аналогичным вызову метода: код, открывший диалоговое окно с использованием <xref:System.Windows.Window.ShowDialog%2A> Waits до <xref:System.Windows.Window.ShowDialog%2A> возврата. При <xref:System.Windows.Window.ShowDialog%2A> возврате код, вызвавший его, должен решить, следует ли продолжать обработку или прекратить обработку в зависимости от того, нажал ли пользователь кнопку **ОК** или кнопку **Отмена** . Чтобы упростить это решение, диалоговое окно должно вернуть выбор пользователя в качестве <xref:System.Boolean> значения, возвращаемого <xref:System.Windows.Window.ShowDialog%2A> методом.  

При нажатии кнопки **ОК** <xref:System.Windows.Window.ShowDialog%2A> должен возвращаться `true` . Это достигается путем задания <xref:System.Windows.Window.DialogResult%2A> Свойства диалогового окна при нажатии кнопки **ОК** .  

[!code-csharp[Responding to the OK button](~/samples/snippets/csharp/VS_Snippets_Wpf/DialogBoxSample/CSharp/MarginsDialogBox.xaml.cs?range=1-8,25-27,32-33,67-68)]
[!code-vb[Responding to the OK button](~/samples/snippets/visualbasic/VS_Snippets_Wpf/DialogBoxSample/VisualBasic/MarginsDialogBox.xaml.vb?range=1-8,27,31-33,61-62)]  

Обратите внимание, что установка <xref:System.Windows.Window.DialogResult%2A> свойства также приводит к автоматическому закрытию окна, что позволяет устранить необходимость явного вызова <xref:System.Windows.Window.Close%2A> .  
  
При нажатии кнопки **Отмена** <xref:System.Windows.Window.ShowDialog%2A> должна возвращаться `false` , что также требует установки <xref:System.Windows.Window.DialogResult%2A> Свойства.  
  
[!code-csharp[Responding to the Cancel button](~/samples/snippets/csharp/VS_Snippets_Wpf/DialogBoxSample/CSharp/MarginsDialogBox.xaml.cs?range=1-8,19-24,67-68)]
[!code-vb[Responding to the Cancel button](~/samples/snippets/visualbasic/VS_Snippets_Wpf/DialogBoxSample/VisualBasic/MarginsDialogBox.xaml.vb?range=1-8,22-25,61-62)]  

Если <xref:System.Windows.Controls.Button.IsCancel%2A> свойство кнопки имеет значение `true` и пользователь нажимает кнопку « **Отмена** » или клавишу ESC, автоматически устанавливается значение <xref:System.Windows.Window.DialogResult%2A> `false` . Следующая разметка действует аналогично предыдущему коду без необходимости в обработке <xref:System.Windows.Controls.Primitives.ButtonBase.Click> события.  
...
Если параметр <xref:System.Windows.Window.DialogResult%2A> задается в диалоговом окне, то функция, которая открыла ее, может получить результат диалогового окна, проверив <xref:System.Windows.Window.DialogResult%2A> свойство при <xref:System.Windows.Window.ShowDialog%2A> возврате.  
