...
- Входные данные находятся в отдельном потоке из WPF <xref:System.Windows.Controls.WebBrowser> , поэтому ввод с клавиатуры не может быть перехвачен, а состояние IME не является общим.
...
- <xref:System.Windows.Interop.HwndHost.MessageHook>не вызывается, так как <xref:System.Windows.Interop.HwndHost> не может создать подкласс окна, выполняющегося в другом потоке или процессе.
