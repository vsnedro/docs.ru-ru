...
3. **Добавить основные кнопки по умолчанию:** Все файлы, необходимые для этого пошагового руководства, предоставляются шаблоном. Откройте файл Window1. XAML, дважды щелкнув его в обозреватель решений. По умолчанию <xref:System.Windows.Controls.Grid> в Window1. XAML присутствует элемент. Удалите <xref:System.Windows.Controls.Grid> элемент и добавьте на страницу несколько кнопок [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] , введя или скопировав и вставляя следующий выделенный код в Window1. XAML:
...
1. **Определите блок Application. Resources:** Откройте App. XAML и добавьте следующую выделенную разметку, если она еще не существует:
    ...
    ```
     ...
     Область ресурсов определяется местом, где определяется ресурс. Определение ресурсов в `Application.Resources` файле App. XAML позволяет использовать ресурс из любого места в приложении. Дополнительные сведения об определении области ресурсов см. в разделе [ресурсы XAML](../../../desktop-wpf/fundamentals/xaml-resources-define.md).

2. **Создайте стиль и определите в нем основные значения свойств:** Добавьте в блок следующую разметку `Application.Resources` . Эта разметка создает объект <xref:System.Windows.Style> , который применяется ко всем кнопкам в приложении, присвоив параметрам для <xref:System.Windows.FrameworkElement.Width%2A> кнопок значение 90 и значение <xref:System.Windows.FrameworkElement.Margin%2A> 10.
    ...
    ```
     ...
     <xref:System.Windows.Style.TargetType%2A>Свойство указывает, что стиль применяется ко всем объектам типа <xref:System.Windows.Controls.Button> . Каждый <xref:System.Windows.Setter> из них задает другое значение свойства для <xref:System.Windows.Style> . Таким образом, на этом этапе каждая кнопка в приложении имеет ширину 90 и поле, равное 10.  Если нажать клавишу F5 для запуска приложения, появится следующее окно.
...
1. **Настройте шаблон:** Так как элементы управления, такие как <xref:System.Windows.Controls.Button> <xref:System.Windows.Controls.Control.Template%2A> , имеют свойство, можно определить значение свойства шаблона так же, как и другие значения свойств, заданные в <xref:System.Windows.Style> с помощью <xref:System.Windows.Setter> . Добавьте следующую выделенную разметку в стиль кнопки.
...
2. **Изменить представление кнопки:** На этом этапе необходимо определить шаблон. Добавьте следующую выделенную разметку. Эта разметка определяет два <xref:System.Windows.Shapes.Rectangle> элемента с закругленными краями, за которыми следует <xref:System.Windows.Controls.DockPanel> . <xref:System.Windows.Controls.DockPanel>Используется для размещения <xref:System.Windows.Controls.ContentPresenter> кнопки. <xref:System.Windows.Controls.ContentPresenter>Отображает содержимое кнопки. В этом пошаговом руководстве содержимое является текстом ("Кнопка 1", "Кнопка 2", "Кнопка 3"). Все компоненты шаблона (прямоугольники и <xref:System.Windows.Controls.DockPanel> ) размещаются внутри <xref:System.Windows.Controls.Grid> .
...
3. **Добавьте глассеффект в шаблон:** Далее вы добавите стекло. Сначала необходимо создать ресурсы, которые создают эффект прозрачного градиента. Добавьте эти ресурсы градиента в любом месте `Application.Resources` блока:
    ...
    ```
     ...
     Эти ресурсы используются в качестве <xref:System.Windows.Shapes.Shape.Fill%2A> для прямоугольника, который мы вставляем в <xref:System.Windows.Controls.Grid> шаблон кнопки. Добавьте в шаблон следующую выделенную разметку.
...
Простой способ добавления интерактивности (нажатие кнопки мыши, выхода из мыши, щелчка и т. д.) заключается в определении триггеров в шаблоне или стиле. Чтобы создать объект <xref:System.Windows.Trigger> , определите свойство "Condition", например: <xref:System.Windows.UIElement.IsMouseOver%2A> значение свойства кнопки равно `true` . Затем определяются методы задания (Actions), которые выполняются, когда условие триггера имеет значение true.
...
4. **Добавить анимацию для** <xref:System.Windows.UIElement.MouseEnter> **и** <xref:System.Windows.UIElement.MouseLeave> **:** Далее мы добавим несколько анимаций к триггерам.   Добавьте следующую разметку в любое место внутри `ControlTemplate.Triggers` блока.
    ...
    ```
     ...
     Существует две анимации, которые запускаются при наведении указателя мыши на кнопку ( <xref:System.Windows.UIElement.MouseEnter> событие вызывается). Эти анимации сжимают стеклянный прямоугольник вдоль осей X и Y. Обратите внимание на свойства <xref:System.Windows.Media.Animation.DoubleAnimation> элементов — <xref:System.Windows.Media.Animation.Timeline.Duration%2A> и <xref:System.Windows.Media.Animation.DoubleAnimation.By%2A> . <xref:System.Windows.Media.Animation.Timeline.Duration%2A>Указывает, что анимация выполняется в течение половины секунды и <xref:System.Windows.Media.Animation.DoubleAnimation.By%2A> указывает, что стекло сжимается на 10%.
...
- Нацелен на <xref:System.Windows.Style> тип объекта ( <xref:System.Windows.Controls.Button> ).
