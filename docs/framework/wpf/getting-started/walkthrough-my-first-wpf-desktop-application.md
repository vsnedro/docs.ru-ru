...
> [!TIP]
> Пример кода, который используется в этом учебнике, доступен как для Visual Basic, так и для C# в [учебнике пример кода приложения WPF](https://github.com/Microsoft/WPF-Samples/tree/master/Getting%20Started/WalkthroughFirstWPFApp).
>
...
4. Измените <xref:System.Windows.Window> элемент на <xref:System.Windows.Navigation.NavigationWindow> , как показано в следующем коде XAML:
   ...
   Это приложение переходит к другому содержимому в зависимости от введенных пользователем данных. Именно поэтому необходимо изменить основное значение <xref:System.Windows.Window> на <xref:System.Windows.Navigation.NavigationWindow> . <xref:System.Windows.Navigation.NavigationWindow>наследует все свойства <xref:System.Windows.Window> . <xref:System.Windows.Navigation.NavigationWindow>Элемент в файле XAML создает экземпляр <xref:System.Windows.Navigation.NavigationWindow> класса. Дополнительные сведения см. в разделе [Общие сведения о навигации](../app-development/navigation-overview.md).

5. Удалите <xref:System.Windows.Controls.Grid> элементы из между <xref:System.Windows.Navigation.NavigationWindow> тегами.

6. Измените следующие свойства в коде XAML для <xref:System.Windows.Navigation.NavigationWindow> элемента:

    - Присвойте <xref:System.Windows.Window.Title%2A> свойству значение " `ExpenseIt` ".

    - Задайте <xref:System.Windows.FrameworkElement.Height%2A> для свойства значение 350 пикселей.

    - Задайте <xref:System.Windows.FrameworkElement.Width%2A> для свойства значение 500 пикселей.
...
8. Если вы используете C#, измените `MainWindow` класс на производный от <xref:System.Windows.Navigation.NavigationWindow> . (В Visual Basic это происходит автоматически при изменении окна в XAML.) Код C# теперь должен выглядеть следующим образом:
...
1. Присвойте свойству значение <xref:System.Windows.Controls.Page.Title%2A> " `ExpenseIt - Home` ".

1. Задайте для параметра значение `DesignHeight` 350 пикселей, а для параметра — значение `DesignWidth` 500 пикселей.
...
1. Добавьте <xref:System.Windows.Navigation.NavigationWindow.Source%2A> свойство в <xref:System.Windows.Navigation.NavigationWindow> элемент и задайте для него значение " `ExpenseItHome.xaml` ".
...
1. Присвойте свойству значение <xref:System.Windows.Controls.Page.Title%2A> " `ExpenseIt - View Expense` ".
...
1. Добавьте в проект образ с именем *watermark.png* . Вы можете создать собственный образ, скопировать файл из примера кода или получить его из репозитория GitHub [Microsoft/WPF-Samples](https://raw.githubusercontent.com/microsoft/WPF-Samples/master/Getting%20Started/WalkthroughFirstWPFApp/csharp/watermark.png) .
...
> [!TIP]
> Дополнительные сведения об <xref:System.Windows.Controls.Panel> элементах см. в разделе [Общие сведения о панелях](../controls/panels-overview.md). Дополнительные сведения о макете см. в разделе [Layout](../advanced/layout.md).
...
1. В *`ExpenseItHome.xaml`* Задайте для <xref:System.Windows.FrameworkElement.Margin%2A> свойства <xref:System.Windows.Controls.Grid> элемента значение "10, 0, 10, 10", которое соответствует левому, верхнему, правому и нижнему полям:
...
2. Добавьте следующий код XAML между <xref:System.Windows.Controls.Grid> тегами, чтобы создать определения строк и столбцов:
    ...
    Для <xref:System.Windows.Controls.RowDefinition.Height%2A> двух строк устанавливается значение <xref:System.Windows.GridLength.Auto%2A> , которое означает, что размер строк зависит от содержимого строк. По умолчанию <xref:System.Windows.Controls.RowDefinition.Height%2A> используется <xref:System.Windows.GridUnitType.Star> размер, что означает, что высота строки является взвешенной пропорциями доступного пространства. Например, если две строки имеют <xref:System.Windows.Controls.RowDefinition.Height%2A> "*", каждая из них имеет высоту, равную половине доступного пространства.

    <xref:System.Windows.Controls.Grid>Теперь должен содержаться следующий код XAML:

    [!code-xaml[ExpenseIt#9](~/samples/snippets/csharp/VS_Snippets_Wpf/ExpenseIt/CSharp/ExpenseIt3/ExpenseItHome.xaml#9)]

## <a name="add-controls"></a>Добавление элементов управления

В этом разделе вы обновите пользовательский интерфейс домашней страницы, чтобы отобразить список людей, в которых вы выбрали одного пользователя для отображения отчета о расходах. Элементы управления — это объекты пользовательского интерфейса, позволяющие пользователям взаимодействовать с приложением. Дополнительные сведения см. в разделе [элементы управления](../controls/index.md).
...
Каждый элемент управления помещается в строку объекта <xref:System.Windows.Controls.Grid> путем установки <xref:System.Windows.Controls.Grid.Row%2A?displayProperty=nameWithType> присоединенного свойства. Дополнительные сведения о вложенных свойствах см. в разделе [Общие сведения о вложенных свойствах](../advanced/attached-properties-overview.md).

1. В *`ExpenseItHome.xaml`* добавьте в тег следующий код XAML между <xref:System.Windows.Controls.Grid> тегами:
...
1. В *`ExpenseItHome.xaml`* добавьте еще один столбец в <xref:System.Windows.Controls.Grid.ColumnDefinitions%2A> с фиксированным <xref:System.Windows.Controls.ColumnDefinition.Width%2A> 230 пикселов:
...
5. Задайте <xref:System.Windows.Controls.Panel.Background%2A?displayProperty=nameWithType> для свойства файл изображения *watermark.png* , ДОБАВИВ следующий код XAML в любое место между `<Grid>` тегами и `</Grid>` :

    [!code-xaml[ExpenseIt#14](~/samples/snippets/csharp/VS_Snippets_Wpf/ExpenseIt/CSharp/ExpenseIt5/ExpenseItHome.xaml#14)]

6. Перед <xref:System.Windows.Controls.Border> элементом добавьте <xref:System.Windows.Controls.Label> с содержимым "Просмотр отчета о расходах". Эта метка является заголовком страницы.
...
1. В *`ExpenseItHome.xaml`* добавьте <xref:System.Windows.Controls.Primitives.ButtonBase.Click> к <xref:System.Windows.Controls.Button> элементу обработчик событий. Дополнительные сведения см. [в разделе инструкции. Создание простого обработчика событий](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2010/bb675300(v=vs.100)).
...
3. В *`ExpenseItHome.xaml`* замените все между <xref:System.Windows.Controls.Grid> элементами следующим кодом XAML:
...
5. Замените все элементы на <xref:System.Windows.Controls.Grid> следующий код XAML:
...
2. В `<Grid.Resources>` элементе добавьте следующий `<xref:System.Windows.DataTemplate>` элемент, определяющий способ отображения данных в <xref:System.Windows.Controls.ListBox> , после `<XmlDataProvider>` элемента:
...
3. Замените существующий <xref:System.Windows.Controls.ListBox> код XAML следующим:
    ...
    Этот XAML привязывает <xref:System.Windows.Controls.ItemsControl.ItemsSource%2A> свойство объекта <xref:System.Windows.Controls.ListBox> к источнику данных и применяет шаблон данных в качестве <xref:System.Windows.Controls.ItemsControl.ItemTemplate%2A> .
...
4. Измените <xref:System.Windows.Controls.Primitives.ButtonBase.Click> обработчик событий, чтобы вызвать новый конструктор, передающий данные отчета о затратах выбранного человека.
...
4. Замените <xref:System.Windows.Controls.DataGridTextColumn> элементы <xref:System.Windows.Controls.DataGridTemplateColumn> <xref:System.Windows.Controls.DataGrid> элементом в элементе и примените к ним шаблоны.
...
- [Общие сведения о XAML (WPF)](../../../desktop-wpf/fundamentals/xaml.md)
