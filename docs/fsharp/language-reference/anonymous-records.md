...
В следующих примерах демонстрируется синтаксис анонимных записей. Элементы разделяются как `[item]` необязательные.
...
Анонимные записи лучше рассматривать как типы записей F #, которые не нужно объявлять перед созданием экземпляра.
...
В следующем примере предыдущий объект разворачивается с помощью `printCircleStats` функции, которая принимает анонимную запись в качестве входных данных:
...
Вызов `printCircleStats` с любым типом анонимных записей, который не имеет такой же "формы", как и тип входных данных, не будет компилироваться:
...
Анонимные записи также можно определить как структуру с `struct` ключевым словом Optional. В следующем примере предыдущий объект расширяется путем создания и использования анонимной записи структуры:
...
Анонимные записи структуры также позволяют «выведение структуры», где не нужно указывать `struct` ключевое слово в месте вызова. В этом примере вы елиде `struct` ключевое слово при вызове `printCircleStats` :
...
Обратный шаблон — указывает, `struct` когда входной тип не является анонимной записью структуры, не сможет компилироваться.
...
`x`Значения и `y` имеют разные типы и несовместимы друг с другом. Они не являются сопоставимыми и не являются сравнимыми. Чтобы проиллюстрировать это, рассмотрим эквивалент именованной записи:
...
Можно выполнять сериализацию анонимных записей так же, как и с именованными записями. Ниже приведен пример использования [Newtonsoft.Jsв](https://www.nuget.org/packages/Newtonsoft.Json/):
...
Можно использовать API-интерфейс .NET, который требует использования [анонимных типов C#](../../csharp/programming-guide/classes-and-structs/anonymous-types.md). Анонимные типы C# являются тривиальными для взаимодействия с с помощью анонимных записей. В следующем примере показано, как использовать анонимные записи для вызова перегрузки [LINQ](../../csharp/programming-guide/concepts/linq/index.md) , для которой требуется анонимный тип:
...
Нельзя объявлять анонимные записи структуры как `IsByRefLike` или `IsReadOnly` . Существует [предложение Open Language](https://github.com/fsharp/fslang-suggestions/issues/712) для `IsByRefLike` `IsReadOnly` анонимных записей.
