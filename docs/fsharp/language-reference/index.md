...
|Название|Описание:|
|-----|-----------|
|[Справочные сведения о ключевых словах](keyword-reference.md)|Содержит ссылки на сведения обо всех ключевых словах языка F#.|
...
|[Соответствие шаблону](pattern-matching.md)|Описывает шаблоны, которые являются правилами для преобразования входных данных и используются во всем языке F#. Пользователь может сравнить данные с шаблоном, разбить данные на составляющие части или извлечь сведения из данных теми или иными способами.|
...
|[Проверочные утверждения](assertions.md)|Описывает выражение `assert`, являющееся функцией отладки, которую можно использовать для тестирования выражения. При сбое в режиме отладки утверждение создает диалоговое окно системной ошибки.|
|[Обработка исключений](./exception-handling/index.md)|Содержит сведения о поддержке обработки исключений в языке F#.|
|[attributes](attributes.md)|Описывает атрибуты, позволяющие применять метаданные к программным конструкциям.|
...
|[namespaces](namespaces.md)|Описывает поддержку пространств имен в F#. С помощью пространства имен вы можете упорядочить код по областям соответствующей функциональности, подключив имя к группированию элементов программы.|
...
|[Документация XML](xml-documentation.md)|Описывает создание файлов документации для комментариев к XML-документам. Документацию можно создавать из комментариев к коду в F#, так же как и в других языках .NET.|
...
|[Форматирование обычного текста](plaintext-formatting.md)|Узнайте, как использовать sprintf и другое форматирование обычного текста в приложениях и сценариях F#.|
...
|[Базовые типы](basic-types.md)|Описывает фундаментальные базовые типы, используемые в языке F#. Кроме того, указывает соответствующие типы .NET и минимальное и максимальное значения для каждого из них.|
|[Тип Unit](unit-type.md)|Описывает тип `unit`, который указывает на отсутствие конкретного значения. Тип `unit` имеет только одно значение, которое выступает в качестве заполнителя, если другое значение не существует или не требуется.|
...
|[Сокращенные обозначения типов](type-abbreviations.md)|Описывает сокращенные формы типов, которые являются альтернативными именами типов.|
...
|[Отложенные выражения](lazy-expressions.md)|Описывает отложенные выражения, которые выполняются не сразу, а только тогда, когда возникает потребность в результате.|
