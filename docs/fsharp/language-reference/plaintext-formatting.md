...
| Описатель формата   | Типы (s)        | Примечания                      |
|:-------------------|:---------------|:-----------------------------|
...
| `%O` | любое значение  |   Отформатировано с помощью упаковки-преобразования объекта и валлинг его `System.Object.ToString()` метод |
...
| `%a` | любое значение  |   Требуется два аргумента — функция форматирования, принимающая параметр контекста и значение, а также конкретное значение для печати. |
...
Необязательная ширина — это целое число, указывающее минимальную ширину результата. Например, `%6d` выводит целое число, предваряя его пробелами, чтобы заполнить не менее 6 символов. Если задано значение Width `*` , то для указания соответствующей ширины берется дополнительный целочисленный аргумент.

Допустимые флаги:

| Flag   | Действие        | Примечания                      |
|:-------------------|:---------------|:-----------------------------|
...
При форматировании обычного текста с помощью `%A` спецификатора для списков и кортежей F # используется отступ блока. Показан в предыдущем примере.
...
Если задать ширину печати, равную 0, то ширина печати не будет использоваться. Будет выведена одна строка текста, за исключением тех случаев, когда встроенные строки в самих выходных данных содержат линебреакс.  Пример
...
Отступ блока также используется для структуры значений общедоступной записи и объединения. например следующие.
...
Значения NULL распечатываются, как `null` если бы статический тип значения не определялся как тип объединения, где `null` является разрешенным репресенатион.
