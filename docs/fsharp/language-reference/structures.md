...
Можно закомментировать структуры с помощью <xref:System.Runtime.CompilerServices.IsReadOnlyAttribute> атрибута. Пример:
...
Использование этого атрибута создает метаданные, позволяя F # и C# обрабатывали его как `inref<'T>` и `in ref` соответственно.
