...
Некоторые функции XML-документации на C# и другие языки .NET не поддерживаются в C#.

- В F # перекрестные ссылки должны использовать полную XML-подпись соответствующего символа, например `cref="T:System.Console"` .
  Простые перекрестные ссылки в стиле C#, такие как `cref="Console"` , не изменяются до полных сигнатур XML, и эти элементы не проверяются компилятором F #. Некоторые средства документирования могут разрешать использование этих перекрестных ссылок последующей обработкой, но следует использовать полные подписи.
