...
Как видите, `main` функция содержит еще несколько элементов. По сути, он выполняет следующие действия:

1. Преобразуйте аргументы командной строки в последовательность `Async<unit>` вычислений с помощью `Seq.map` .
2. Создайте `Async<'T[]>` , который планирует и выполняет `printTotalFileBytes` вычисления параллельно при запуске.
3. Создайте объект `Async<unit>` , который будет выполнять параллельное вычисление и игнорировать его результат (то есть `unit[]` ).
4. Явным образом запустите общие составные вычисления с `Async.RunSynchronously` , блокируя до завершения.
...
Это будет запланировать `printTotalFileBytes` выполнение в порядке элементов, `argv` а не планировать их параллельно. Так как каждая последовательная операция не будет планироваться до тех пор, пока не завершится выполнение предыдущего вычисления, вычисления будут упорядочены таким, что в их выполнении не будет перекрываться.

## <a name="important-async-module-functions"></a>Важные функции асинхронного модуля

При написании асинхронного кода в F # вы обычно взаимодействуете с платформой, которая обрабатывает планирование вычислений. Однако это не всегда так, поэтому лучше понять различные функции, которые можно использовать для планирования асинхронной работы.
...
- Когда необходимо вызвать API .NET, который возвращает, <xref:System.Threading.Tasks.Task%601> чтобы представить результат асинхронного вычисления.
...
Планирует выполнение последовательности асинхронных вычислений параллельно, возвращая массив результатов в том порядке, в котором они были предоставлены. Степень параллелизма может при необходимости настраиваться и регулироваться путем указания `maxDegreeOfParallelism` параметра.
...
- Исключения упаковываются в соответствии с <xref:System.AggregateException> соглашением о библиотеке параллельных задач. это поведение отличается от того, как в F # Async обычно отображаются исключения.
...
Создает асинхронное вычисление, которое выполняет заданное вычисление, но удаляет его результат.
...
- При наличии асинхронного вычисления, результат которого не требуется. Это аналогично `ignore` функции для неасинхронного кода.
...
Выполняет асинхронное вычисление и ожидает его результат в вызывающем потоке. Распространяет исключение, если вычисления возвращают один. Этот вызов блокируется.
...
Запускает асинхронное вычисление, которое возвращает `unit` в пуле потоков. Не ждет его завершения и (или) следит за результатом исключения. Вложенные вычисления, запущенные с `Async.Start` , запускаются независимо от родительских вычислений, которые их вызвали; их время существования не привязано к какому-либо родительскому вычислению. Если родительские вычисления отменены, дочерние вычисления не отменяются.
...
- У вас нет необходимости иметь в виду или сообщать об исключениях, возникших в результате выполнения.
