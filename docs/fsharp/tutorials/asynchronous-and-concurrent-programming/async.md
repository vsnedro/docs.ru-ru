...
Хотя фоновая работа часто включает в себя использование нескольких потоков, важно рассматривать понятия асинхронности и многопоточности отдельно. На самом деле, они являются отдельными проблемами, и одно не подразумевает другое. В этой статье описаны отдельные понятия более подробно.
...
Все три ортогональных концепций, но могут быть легко conflated, особенно когда они используются вместе. Например, может потребоваться параллельно еранды несколько асинхронных вычислений. Эта взаимосвязь не означает, что параллелизм или асинхронность подразумевают друг друга.
...
4. Явно запускать последние `Async.RunSynchronously` вычисления с и блокировать, пока он не завершится.
...
- Вычисления будут запускаться всякий раз, когда они в конечном итоге получить запланировано. Такое поведение означает, что вы не можете полагаться на их порядок их выполнения.
...
- Исключения заворачиваются <xref:System.AggregateException> в соответствии с конвенцией Параллельной библиотеки задач, и это поведение отличается от того, как F'async обычно всплывает на поверхность исключений.
...
- Если вы `Async.Ignore` должны использовать, `Async.Start` потому что `Async<unit>`вы хотите использовать или другую функцию, которая требует, рассмотреть, если отбрасывание результат в порядке. Избегайте отбрасывания результатов только для того, чтобы соответствовать подписи типа.
...
- Любая работа (например, вызов), `printfn`начатая с `Async.Start` не приведет к тому, что эффект произойдет на основной поток выполнения программы.
