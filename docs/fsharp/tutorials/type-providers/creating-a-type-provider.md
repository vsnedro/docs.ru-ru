...
Экосистема F # содержит ряд поставщиков типов для часто используемых служб Интернет-и корпоративных данных. Например:
...
- [FSharp. Data. SqlClient](https://fsprojects.github.io/FSharp.Data.SqlClient/) имеет набор поставщиков типов для внедрения T-SQL во время компиляции в F #.
...
Этот тип должен быть открытым, и его необходимо пометить атрибутом [типепровидер](https://msdn.microsoft.com/library/bdf7b036-7490-4ace-b79f-c5f1b1b37947) , чтобы компилятор распознал поставщик типов, если отдельный проект F # ссылается на сборку, содержащую тип. Параметр *config* является необязательным, и, если он имеется, содержит контекстные сведения о конфигурации для экземпляра поставщика типов, создаваемого компилятором F #.
...
При получении этого свойства всегда будет вычислена строка "Hello!". `GetterCode`Для свойства используется цитата F #, представляющая код, формируемый компилятором хоста для получения свойства. Дополнительные сведения о предложениях см. в разделе [цитирование кода (F #)](https://msdn.microsoft.com/library/6f055397-a1f0-4f9a-927c-f0d7c6951155).
...
- Единицы — это все международные единицы (SI), которые определяются модулем [Microsoft. FSharp. Data. унитсистемс. si. UnitNames (F #)](https://msdn.microsoft.com/library/3cb43485-11f5-4aa7-a779-558f19d4013b) .
