...
Присвойте свойству <xref:System.Web.Configuration.HttpRuntimeSection.EnableHeaderChecking> значение `false`.
...
Присвойте свойству <xref:System.Web.Configuration.HttpRuntimeSection.EnableHeaderChecking> значение `true`. Или удалите назначение, `false` так как значение по умолчанию — `true` .
