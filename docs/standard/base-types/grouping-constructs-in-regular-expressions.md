...
- Применение квантификатору к части выражения с несколькими языковыми элементами регулярных выражений. Дополнительные сведения о квантификаторах см. в разделе [Quantifiers](quantifiers-in-regular-expressions.md).  
...
|10|`(((?'Open'<)`|Соответствует левой угловой скобке в "\<mno" and assigns it to the `Open` group. Its <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=nameWithType> коллекция теперь имеет единственное значение "<".|  
...
|13|`(((?'Open'<)`|Сопоставляется левая угловая скобка в строке "\<xyz>", которая назначается группе `Open`. Коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=nameWithType> группы `Open` теперь содержит два символа: левую угловую скобку из "\<mno", and the left angle bracket from "\<xyz>".|  
...
|16|`((?'Close-Open'>)`|Сопоставляется правая угловая скобка в "\<xyz>". "xyz" задает подстроку между группой `Open` и правой угловой скобкой группы `Close` и удаляет текущее значение группы `Open` . Значение предыдущего символа (левая угловая скобка в "\<mno") becomes the current value of the `Open` group. The <xref:System.Text.RegularExpressions.Group.Captures%2A> коллекция группы `Open` теперь содержит один символ: левую угловую скобку из "\<xyz>".|  
...
## <a name="group-options"></a>Параметры группы  
 ...
 Где *часть выражения* — любой допустимый шаблон регулярного выражения. Например, `(?i-s:)` отключает учет регистра и однострочный режим. Дополнительные сведения о доступных встроенных параметрах см. в разделе [Параметры регулярных выражений](regular-expression-options.md).  
...
- Следующий набор объектов <xref:System.Text.RegularExpressions.Group> представляет неименованные (нумерованные) захватываемые группы. Они указываются в том порядке, в котором они определены в регулярном выражении, слева направо. Значения индекса этих групп лежат в диапазоне от 1 до числа неименованных захватываемых групп в коллекции. (Индекс определенной группы эквивалентен нумерованной обратной ссылке. Дополнительные сведения об обратных ссылках см. в разделе [Конструкции обратных ссылок в регулярных выражениях](backreference-constructs-in-regular-expressions.md).)  
