...
|`(?<` *name* `>` *subexpression* `)`<br /> или <br />`(?'` *name* `'` *subexpression* `)`|Выделяет соответствующую часть выражения в именованную группу.|`(?<double>\w)\k<double>`|`"ee"` в `"deep"`|
|`(?<` *name1* `-` *name2* `>` *subexpression* `)` <br /> или <br /> `(?'` *name1* `-` *name2* `'` *subexpression* `)`|Задает сбалансированное определение группы. Дополнительные сведения см. в разделе "Сбалансированное определение группы" статьи [Конструкции группирования](grouping-constructs-in-regular-expressions.md).|`(((?'Open'\()[^\(\)]*)+((?'Close-Open'\))[^\(\)]*)+)*(?(Open)(?!))$`|`"((1-3)*(3-1))"` в `"3+2^((1-3)*(3-1))"`|
