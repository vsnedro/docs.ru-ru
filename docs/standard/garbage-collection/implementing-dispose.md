...
ms.locfileid: "87455740"
---
# <a name="implement-a-dispose-method"></a>Реализация метода Dispose

Реализация метода <xref:System.IDisposable.Dispose%2A> в основном используется для освобождения неуправляемых ресурсов. При работе с членами экземпляра, которые являются реализациями <xref:System.IDisposable>, обычно применяются каскадные вызовы <xref:System.IDisposable.Dispose%2A>. Существуют дополнительные причины для реализации <xref:System.IDisposable.Dispose%2A>, включая освобождение выделенной памяти, удаление добавленного в коллекцию элемента или информирование о снятой блокировке.
...
- [Определение и использование классов и структур (C++/CLI)](/cpp/dotnet/how-to-define-and-consume-classes-and-structs-cpp-cli)
