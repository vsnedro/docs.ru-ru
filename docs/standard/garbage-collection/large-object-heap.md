...
Сборщик мусора .NET (GC) разделяет объекты на две категории — большие и маленькие. Некоторые атрибуты больших объектов становятся более значимыми, чем атрибуты маленьких. Например, для сжатия объекта&mdash;копирования его в другое место в куче&mdash;требуется много ресурсов. Поэтому сборщик мусора помещает большие объекты в кучу больших объектов (LOH). В этой статье мы поговорим о том, что такое большой объект, как собираются большие объекты и как они влияют на производительность.
...
## <a name="how-an-object-ends-up-on-the-loh"></a>Как объект оказывается в куче больших объектов
...
## <a name="collect-performance-data-for-the-loh"></a>Сбор данных производительности для кучи больших объектов
...
Если у вас есть только дамп памяти и вам нужно увидеть, какие объекты находятся в куче, можно использовать [расширение отладчика SoS](../../framework/tools/sos-dll-sos-debugging-extension.md), предоставляемое .NET.
