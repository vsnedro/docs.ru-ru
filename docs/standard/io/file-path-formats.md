...
Чтобы определить, является ли путь к файлу полным (такой путь не зависит от текущего каталога и не изменяется при смене текущего каталога), можно вызвать метод <xref:System.IO.Path.IsPathFullyQualified%2A?displayProperty=nameWithType>. Обратите внимание, что такой путь может включать сегменты с относительным путем к каталогу (`.` и `..`), но при этом по-прежнему будет полным, если разрешенный путь всегда указывает на одно и то же место.
...
### <a name="identify-the-path"></a>Идентификация пути
...
### <a name="handle-legacy-devices"></a>Работа с устаревшими устройствами
...
### <a name="apply-the-current-directory"></a>Применение текущего каталога
...
### <a name="canonicalize-separators"></a>Канонизация разделителей
...
### <a name="evaluate-relative-components"></a>Вычисление относительных компонентов
...
### <a name="trim-characters"></a>Удаление знаков
...
## <a name="skip-normalization"></a>Пропуск нормализации
