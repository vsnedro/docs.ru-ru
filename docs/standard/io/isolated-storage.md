...
.NET Framework и .NET Core предлагают [изолированное хранилище](/dotnet/standard/io/isolated-storage) в качестве механизма сохранения данных для пользователя, приложения или компонента. Это устаревший компонент, изначально разрабатывавшийся для сценариев управления доступом для кода, которые сейчас объявлены нерекомендуемыми.

Для считывания данных через границы доверия можно использовать различные средства и интерфейсы API изолированного хранилища. Например, считывание данных из области на уровне компьютера может агрегировать данные из других, возможно, менее доверенных учетных записей пользователей на компьютере. Компоненты или приложения, которые считывают данные из областей изолированного хранилища на уровне компьютера, должны быть осведомлены о последствиях такого чтения.

### <a name="security-sensitive-apis-which-can-read-from-the-machine-wide-scope"></a>Чувствительные к безопасности API, которые могут считывать данные из области на уровне компьютера
...
[Средство изолированного хранилища](/dotnet/framework/tools/storeadm-exe-isolated-storage-tool) `storeadm.exe` используется при вызове с параметром `/machine`, как показано в следующем коде.
...
__Примечание.__ Чтобы произвести подобную атаку, Мэллори потребуется следующее.

* Учетная запись пользователя на компьютере.
* Возможность поместить файл в известное расположение в файловой системе.
* Знание того, что в некоторый момент Боб запустит приложение, которое попытается считать эти данные.
