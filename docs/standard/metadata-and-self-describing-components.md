...
В приведенном ниже примере показана часть кода MSIL, полученного для функции `Main` из предыдущего кода. Код MSIL и метаданные любого приложения .NET можно просмотреть при помощи [дизассемблера MSIL (Ildasm.exe)](../framework/tools/ildasm-exe-il-disassembler.md).
