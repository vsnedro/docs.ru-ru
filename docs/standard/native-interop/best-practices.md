...
- ✔️ РЕКОМЕНДУЕТСЯ использовать только атрибуты `[In]` и `[Out]`, если требуемая реакция на событие отличается от реакции по умолчанию.
- ✔️ ДОПУСТИМО использовать <xref:System.Buffers.ArrayPool%601?displayProperty=nameWithType> для заполнения буферов нативного массива.
...
2. Вызвать
   ...
   3. В только что выделенный управляемый массив копируется собственный буфер, если задано значение `[Out]` **{3}**  _(также значение по умолчанию для `StringBuilder`)_ .
...
> __Для среды Windows__. Для строк `[Out]` в среде CLR по умолчанию используется `CoTaskMemFree` (для свободных строк) или `SysStringFree` (для строк, обозначенных как `UnmanagedType.BSTR`).
> **В большинстве API с буфером выходной строки:** Количество переданных символов должно включать значение NULL. Если возвращаемое значение меньше, чем количество переданных символов, значит, вызов завершился успешно. В таком случае значение — это количество символов *без* NULL в конце. В противном случае это требуемый размер буфера, *включая* нуль-символ.
>
...
❌ НЕЛЬЗЯ использовать `[MarshalAs(UnmanagedType.LPStruct)]` для параметров, за исключением параметров GUID `ref`.
...
Поскольку <xref:System.Delegate?displayProperty=fullName> и <xref:System.MulticastDelegate?displayProperty=fullName> не имеют требуемых сигнатур, они не гарантируют согласованность переданного делегата с сигнатурой, которую ожидает машинный код. Кроме того, в .NET Framework и .NET Core маршалинг структуры, которая содержит `System.Delegate` или `System.MulticastDelegate`, из собственного представления в управляемый объект может привести к нестабильности во время выполнения, если значение этого поля в собственном представлении не является указателем на функцию, который является оболочкой для управляемого делегата. В .NET 5 и более поздних версиях не поддерживается маршалинг поля `System.Delegate` или `System.MulticastDelegate` из собственного представления в управляемый объект. Используйте вместо `System.Delegate` или `System.MulticastDelegate` конкретный тип делегата.
