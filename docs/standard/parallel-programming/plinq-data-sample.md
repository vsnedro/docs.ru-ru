...
ms.locfileid: "94830575"
---
# <a name="plinq-data-sample"></a><span data-ttu-id="698a2-102">Пример данных PLINQ</span><span class="sxs-lookup"><span data-stu-id="698a2-102">PLINQ Data Sample</span></span>
<span data-ttu-id="698a2-103">Этот пример содержит тестовые данные в формате CSV и несколько методов, которые преобразуют эти данные в коллекции в памяти (клиенты, продукты, заказы и сведения о заказе).</span><span class="sxs-lookup"><span data-stu-id="698a2-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="698a2-104">Чтобы поэкспериментировать с PLINQ, вы можете вставить в этот код примеры кода из других статей и вызвать его из метода `Main`.</span><span class="sxs-lookup"><span data-stu-id="698a2-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="698a2-105">Также вы можете применить собственные запросы PLINQ к этим данным.</span><span class="sxs-lookup"><span data-stu-id="698a2-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="698a2-106">Данные представляют собой подмножество базы данных Northwind.</span><span class="sxs-lookup"><span data-stu-id="698a2-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="698a2-107">Сюда включены данные из ограниченного набора полей по 50 клиентам.</span><span class="sxs-lookup"><span data-stu-id="698a2-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="698a2-108">Также предоставляются подмножества строк из таблицы заказов (Orders) и соответствующих сведений о заказах (Order_Detail) для каждого клиента.</span><span class="sxs-lookup"><span data-stu-id="698a2-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="698a2-109">Включены все продукты (Products).</span><span class="sxs-lookup"><span data-stu-id="698a2-109">All Products are included.</span></span>  
  
> [!NOTE]
> <span data-ttu-id="698a2-110">Этот набор данных недостаточно велик, чтобы продемонстрировать преимущество PLINQ в скорости по сравнению с запросами LINQ to Objects на примере простых запросов с предложениями `where` и `select`.</span><span class="sxs-lookup"><span data-stu-id="698a2-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="698a2-111">Реальное увеличение скорости для таких небольших наборов данных можно заметить лишь в сложных запросах, требующих ресурсоемких вычислений по каждому элементу набора данных.</span><span class="sxs-lookup"><span data-stu-id="698a2-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="698a2-112">Настройка этого образца</span><span class="sxs-lookup"><span data-stu-id="698a2-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="698a2-113">Создайте проект консольного приложения Visual C# или Visual Basic.</span><span class="sxs-lookup"><span data-stu-id="698a2-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="698a2-114">Замените содержимое файлов Module1.vb или Program.cs кодом, приведенным после следующих шагов.</span><span class="sxs-lookup"><span data-stu-id="698a2-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="698a2-115">В меню **Проект** выберите **Добавить новый элемент**.</span><span class="sxs-lookup"><span data-stu-id="698a2-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="698a2-116">Выберите **Текстовый файл** и щелкните **ОК**.</span><span class="sxs-lookup"><span data-stu-id="698a2-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="698a2-117">Скопируйте данные из этой статьи и вставьте их в новый текстовый файл.</span><span class="sxs-lookup"><span data-stu-id="698a2-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="698a2-118">В меню **Файл** щелкните команду **Сохранить**, введите имя файла Plinqdata.csv и сохраните его в папку, где расположены файлы исходного кода.</span><span class="sxs-lookup"><span data-stu-id="698a2-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="698a2-119">Нажмите клавишу F5 и убедитесь, что проект правильно компилируется и выполняется.</span><span class="sxs-lookup"><span data-stu-id="698a2-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="698a2-120">В окне консоли вы увидите такие выходные данные, как показано ниже.</span><span class="sxs-lookup"><span data-stu-id="698a2-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="698a2-121">Данные</span><span class="sxs-lookup"><span data-stu-id="698a2-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="698a2-122">См. также раздел</span><span class="sxs-lookup"><span data-stu-id="698a2-122">See also</span></span>

- [<span data-ttu-id="698a2-123">Parallel LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="698a2-123">Parallel LINQ (PLINQ)</span></span>](introduction-to-plinq.md)
