...
В следующем примере показан весь процесс создания потока, шифрования потока, чтения из потока и закрытия потока. В этом примере создается файловый поток, зашифрованный с помощью класса **CryptoStream** и класса **AES** . Сообщение записывается в зашифрованный поток при помощи класса <xref:System.IO.StreamWriter> .
...
Код шифрует поток с помощью симметричного алгоритма AES и записывает "Hello World!" в этот поток. Если код выполнен успешно, создается зашифрованный файл с именем *TestData.txt* и на консоль выводится следующий текст:  
  
```console  
The text was encrypted.  
```  

Файл можно расшифровать с помощью примера симметричного расшифровки при [расшифровке данных](decrypting-data.md). Этот пример и этот пример задают один и тот же ключ и вектор инициализации.
