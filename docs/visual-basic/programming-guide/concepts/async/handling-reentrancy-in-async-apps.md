...
> [!NOTE]
> Версия протокола TLS 1.2 теперь является минимальной версией для использования в разработке приложений. Если ваше приложение нацелено на версию .NET Framework раньше 4.7, обратитесь к следующей статье для [безопасности транспортного уровня (TLS) с помощью .NET Framework.](../../../../framework/network-programming/tls.md)

## <a name="recognizing-reentrancy"></a><a name="BKMK_RecognizingReentrancy"></a> Распознавание поддержки повторного входа
...
## <a name="handling-reentrancy"></a><a name="BKMK_HandlingReentrancy"></a>Обработка ретрансции
...
- [Отмена и перезагрузка операции](#BKMK_CancelAndRestart)
...
### <a name="disable-the-start-button"></a><a name="BKMK_DisableTheStartButton"></a>Отключить кнопку "Пуск"
...
### <a name="cancel-and-restart-the-operation"></a><a name="BKMK_CancelAndRestart"></a> Отмена и перезапуск операции
...
Для получения дополнительной информации об отмене [см.](../../../../visual-basic/programming-guide/concepts/async/fine-tuning-your-async-application.md)

Чтобы настроить этот сценарий, внесите следующие изменения в основной код, который содержится в разделе [Проверка и выполнение примера приложения](#BKMD_SettingUpTheExample). Вы также можете скачать готовое приложение из [Async Образцы: Reentrancy в .NET настольных приложений](https://code.msdn.microsoft.com/Async-Sample-Preventing-a8489f06). Этот проект называется CancelAndRestart.
...
2. В `StartButton_Click` определите, выполняется ли операция на данный момент. Если значение `cts` является, `Nothing`ни одна операция уже не активна. Если значение не `Nothing`является, операция, которая уже запущена, отменяется.
...
4. В `StartButton_Click`конце, текущий процесс завершен, поэтому установите значение `cts` обратно. `Nothing`
...
Если вы выбираете кнопку **«Пуск»** несколько раз во время работы этого приложения, она должна дать результаты, напоминающие следующий выход:
...
### <a name="run-multiple-operations-and-queue-the-output"></a><a name="BKMK_RunMultipleOperations"></a>Выполнить несколько операций и очередь вывода
...
Вы можете запустить этот пример, вставив изменения в код в [создании приложения](#BKMK_BuildingTheApp), или вы можете следовать инструкциям в [загрузке приложения,](#BKMK_DownloadingTheApp) чтобы загрузить образец, а затем запустить проект ОчередьResults.
...
- Задача `pendingWork` находится `Nothing` в начале `FinishOneGroupAsync` только для группы А, которая началась в первую очередь. Группа A еще не завершила выражение await, когда она достигает метода `FinishOneGroupAsync`. Таким образом, управление не возвращается `AccessTheWebAsync`, а первое присваивание задаче `pendingWork` не возникает.
...
## <a name="reviewing-and-running-the-example-app"></a><a name="BKMD_SettingUpTheExample"></a>Просмотр и запуск приложения «Пример»
...
### <a name="downloading-the-app"></a><a name="BKMK_DownloadingTheApp"></a> Загрузка приложения
...
### <a name="building-the-app"></a><a name="BKMK_BuildingTheApp"></a> Сборка приложения
...
2. В меню **Файл** последовательно выберите пункты **Создать**, **Проект**.

     Откроется диалоговое окно **Создание проекта** .

3. В **установленной панели шаблонов** расширьте **Visual Basic,** а затем расширьте **Windows.**
...
8. В **Solution Explorer**, правое нажатие на **ссылки** и выберите **Добавить Справка**.

     Добавьте ссылку на <xref:System.Net.Http>, если она еще не выбрана.

9. В **Solution Explorer**откройте меню ярлыка для MainWindow.xaml.vb, а затем выберите View **Code.**
...
- [Пошаговое руководство. Получение доступа к Интернету с помощью модификатора Async и оператора Await (Visual Basic)](../../../../visual-basic/programming-guide/concepts/async/walkthrough-accessing-the-web-by-using-async-and-await.md)
