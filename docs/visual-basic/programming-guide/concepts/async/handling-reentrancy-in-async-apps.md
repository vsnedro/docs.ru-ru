...
## <a name="handling-reentrancy"></a><a name="BKMK_HandlingReentrancy"></a> Обработка поддержки повторного входа
...
### <a name="disable-the-start-button"></a><a name="BKMK_DisableTheStartButton"></a> Отключение кнопки запуска
...
Дополнительные сведения об отмене см. [в разделе тонкая настройка асинхронного приложения (Visual Basic)](fine-tuning-your-async-application.md).

Чтобы настроить этот сценарий, внесите следующие изменения в основной код, который содержится в разделе [Проверка и выполнение примера приложения](#BKMD_SettingUpTheExample). Можно также загрузить готовое приложение из статьи [Примеры асинхронности. Поддержка повторного входа в классических приложениях .NET](https://code.msdn.microsoft.com/Async-Sample-Preventing-a8489f06). Этот проект называется CancelAndRestart.
...
2. В `StartButton_Click` определите, выполняется ли операция на данный момент. Если значение `cts` равно `Nothing` , операция уже не активна. Если значение не равно `Nothing` , операция, которая уже выполняется, отменяется.
...
4. В конце `StartButton_Click` , текущий процесс завершен, поэтому установите значение `cts` обратно на `Nothing` .
...
### <a name="run-multiple-operations-and-queue-the-output"></a><a name="BKMK_RunMultipleOperations"></a> Запуск нескольких операций и постановка выходных данных в очередь
...
Вы можете выполнить этот пример, вставляя изменения в код в процессе [сборки приложения](#BKMK_BuildingTheApp). также можно выполнить инструкции из статьи [Загрузка приложения](#BKMK_DownloadingTheApp) , чтобы скачать пример, а затем запустить проект QueueResults.
...
- `pendingWork`Задача находится `Nothing` в начале `FinishOneGroupAsync` только для группы A, которая была запущена первой. Группа A еще не завершила выражение await, когда она достигает метода `FinishOneGroupAsync`. Таким образом, управление не возвращается `AccessTheWebAsync`, а первое присваивание задаче `pendingWork` не возникает.
...
## <a name="reviewing-and-running-the-example-app"></a><a name="BKMD_SettingUpTheExample"></a> Проверка и выполнение примера приложения
