...
Кроме того, типы **ValueTuple** реализуют <xref:System.Collections.IStructuralComparable> <xref:System.Collections.IStructuralEquatable> интерфейсы и, позволяющие определять компараторы клиентов.
