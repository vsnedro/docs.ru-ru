...
#### <a name="change-description"></a><span data-ttu-id="d83ee-103">Описание изменений</span><span class="sxs-lookup"><span data-stu-id="d83ee-103">Change description</span></span>

<span data-ttu-id="d83ee-104">Ранее входные данные `NaN` для методов <xref:System.Runtime.Intrinsics.X86.Sse> и <xref:System.Runtime.Intrinsics.X86.Sse2> возвращали неверный результат.</span><span class="sxs-lookup"><span data-stu-id="d83ee-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="d83ee-105">Результат также отличался от результата, создаваемого соответствующим методом в классе <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="d83ee-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="d83ee-106">Начиная с .NET 5.0 эти методы правильно обрабатывают входные данные `NaN` и возвращают те же результаты, что и соответствующие методы в классе <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="d83ee-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="d83ee-107">Архитектура по стандартам отрасли потокового SIMD-расширения (SSE) и потокового SIMD-расширения 2 (SSE2) не обеспечивают прямую аппаратную поддержку этих методов сравнения, поэтому они реализуются в программном обеспечении.</span><span class="sxs-lookup"><span data-stu-id="d83ee-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="d83ee-108">Ранее методы были неправильно реализованы и неправильно обрабатывали входные данные `NaN`.</span><span class="sxs-lookup"><span data-stu-id="d83ee-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="d83ee-109">В коде, перенесенном из машинного кода, неправильное поведение может привести к ошибкам.</span><span class="sxs-lookup"><span data-stu-id="d83ee-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="d83ee-110">Для 256-разрядного пути к коду методы также могут давать результаты, отличные от результатов эквивалентных методов в классе <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="d83ee-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="d83ee-111">В качестве примера неправильной работы методов можно реализовать `CompareNotGreaterThan(x,y)` как `CompareLessThanOrEqual(x,y)` для обычных целых чисел.</span><span class="sxs-lookup"><span data-stu-id="d83ee-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="d83ee-112">Однако для входных данных `NaN` эта логика вычислит неверный результат.</span><span class="sxs-lookup"><span data-stu-id="d83ee-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="d83ee-113">`CompareNotLessThan(y,x)` правильно сравнивает числа *и* учитывает входные данные `NaN`.</span><span class="sxs-lookup"><span data-stu-id="d83ee-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="d83ee-114">Представленная версия</span><span class="sxs-lookup"><span data-stu-id="d83ee-114">Version introduced</span></span>

<span data-ttu-id="d83ee-115">5.0, предварительная версия 4</span><span class="sxs-lookup"><span data-stu-id="d83ee-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="d83ee-116">Рекомендованное действие</span><span class="sxs-lookup"><span data-stu-id="d83ee-116">Recommended action</span></span>

- <span data-ttu-id="d83ee-117">Если предыдущее поведение было ошибкой, изменение не требуется.</span><span class="sxs-lookup"><span data-stu-id="d83ee-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="d83ee-118">Если вы хотите сохранить предыдущее поведение, измените соответствующий вызов следующим образом:</span><span class="sxs-lookup"><span data-stu-id="d83ee-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="d83ee-119">Категория</span><span class="sxs-lookup"><span data-stu-id="d83ee-119">Category</span></span>

<span data-ttu-id="d83ee-120">Библиотеки Core .NET</span><span class="sxs-lookup"><span data-stu-id="d83ee-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="d83ee-121">Затронутые API</span><span class="sxs-lookup"><span data-stu-id="d83ee-121">Affected APIs</span></span>
